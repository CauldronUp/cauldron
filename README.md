...
| Area | State |
|---|---|
| Detection engine (Composer, npm, Go modules) | Working. 533 of 828 Recipes are reachable from a dependency |
...
| Conformance suites (`cauldron verify`) | Working. 11151 cases, 4096 of them checked against a live API |
...
That second line is the honest one. Of every Recipe: 11151 cases, 4096 run against
a live account and 7055 not. Documentation-derived cases are worth having,
and they are not the same as watching the provider do it. Adding a `verified:`
date to a case is a claim that someone did.

All 4096 are the cases whose provider can be asked without a key. Every other
...
| `undeclared` | No description is recorded for this Recipe. 574 of the 828, and falling -- see `cauldron discover` |
...
provider publishes nothing -- 574 of the 828 here -- is served exactly as it
...
The table reaches 533 of the 828 Recipes. The other 295 ship and can be named
