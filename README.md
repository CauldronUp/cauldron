...
| Area | State |
|---|---|
| Detection engine (Composer, npm, Go modules) | Working. 533 of 782 Recipes are reachable from a dependency |
...
| Conformance suites (`cauldron verify`) | Working. 10690 cases, 3893 of them checked against a live API |
...
That second line is the honest one. Of every Recipe: 10690 cases, 3893 run against
a live account and 6797 not. Documentation-derived cases are worth having,
and they are not the same as watching the provider do it. Adding a `verified:`
date to a case is a claim that someone did.

All 3893 are the cases whose provider can be asked without a key. Every other
...
| `undeclared` | No description is recorded for this Recipe. 552 of the 782, and falling -- see `cauldron discover` |
...
provider publishes nothing -- 552 of the 782 here -- is served exactly as it
...
The table reaches 533 of the 782 Recipes. The other 249 ship and can be named
