...
| Area | State |
|---|---|
| Detection engine (Composer, npm, Go modules) | Working. 533 of 760 Recipes are reachable from a dependency |
...
| Conformance suites (`cauldron verify`) | Working. 10497 cases, 3809 of them checked against a live API |
...
That second line is the honest one. Of every Recipe: 10497 cases, 3809 run against
a live account and 6688 not. Documentation-derived cases are worth having,
and they are not the same as watching the provider do it. Adding a `verified:`
date to a case is a claim that someone did.

All 3809 are the cases whose provider can be asked without a key. Every other
...
| `undeclared` | No description is recorded for this Recipe. 539 of the 760, and falling -- see `cauldron discover` |
...
provider publishes nothing -- 539 of the 760 here -- is served exactly as it
...
The table reaches 533 of the 760 Recipes. The other 227 ship and can be named
