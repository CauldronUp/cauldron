...
| Conformance suites (`cauldron verify`) | Working. 6440 cases, 3308 of them checked against a live API |
| Spec drift (`cauldron drift`) | Working. 130 Recipes are checked against their provider's own OpenAPI document; 497 name none yet |
...
```
stripe 0.1.0
  13 of 13 cases passed
  4 checked against the real API, 9 from documentation only
  last checked against the real API on 2026-09-05
```

That second line is the honest one. Of every Recipe: 6440 cases, 3308 run against
a live account and 3132 not. Documentation-derived cases are worth having,
and they are not the same as watching the provider do it. Adding a `verified:`
date to a case is a claim that someone did.

All 3308 are the cases whose provider can be asked without a key. Every other
