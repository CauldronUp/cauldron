...
| Area | State |
|---|---|
| Detection engine (Composer, npm, Go modules) | Working. 533 of 836 Recipes are reachable from a dependency |
...
| Conformance suites (`cauldron verify`) | Working. 11233 cases, 4132 of them checked against a live API |
...
That second line is the honest one. Of every Recipe: 11233 cases, 4132 run against
a live account and 7101 not. Documentation-derived cases are worth having,
and they are not the same as watching the provider do it. Adding a `verified:`
date to a case is a claim that someone did.

All 4132 are the cases whose provider can be asked without a key. Every other
...
| `undeclared` | No description is recorded for this Recipe. 581 of the 836, and falling -- see `cauldron discover` |
...
provider publishes nothing -- 581 of the 836 here -- is served exactly as it
...
The table reaches 533 of the 836 Recipes. The other 303 ship and can be named
