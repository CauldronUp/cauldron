...
| Area | State |
|---|---|
| Detection engine (Composer, npm, Go modules) | Working. 275 of 305 Recipes are reachable from a dependency |
...
| Conformance suites (`cauldron verify`) | Working. 3043 cases, 728 of them checked against a live API |
| Spec drift (`cauldron drift`) | Working. 11 Recipes are checked against their provider's own OpenAPI document, 1 provider publishes one this cannot read, and 293 publish none |
...
That second line is the honest one. Of every Recipe: 3043 cases, 728 run against
...
The seven hundred and twenty-eight are the cases whose provider can be asked without a key. Six are
...
| `undeclared` | The provider publishes nothing to check. 293 of the 305 |
...
The table reaches 275 of the 305 Recipes. The other 30 ship and can be named
