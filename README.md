...
| Area | State |
|---|---|
| Detection engine (Composer, npm, Go modules) | Working. 533 of 808 Recipes are reachable from a dependency |
...
| Conformance suites (`cauldron verify`) | Working. 10943 cases, 4000 of them checked against a live API |
...
That second line is the honest one. Of every Recipe: 10943 cases, 4000 run against
a live account and 6943 not. Documentation-derived cases are worth having,
and they are not the same as watching the provider do it. Adding a `verified:`
date to a case is a claim that someone did.

All 4000 are the cases whose provider can be asked without a key. Every other
...
| `undeclared` | No description is recorded for this Recipe. 563 of the 808, and falling -- see `cauldron discover` |
...
provider publishes nothing -- 563 of the 808 here -- is served exactly as it
...
The table reaches 533 of the 808 Recipes. The other 275 ship and can be named
