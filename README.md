...
| Area | State |
|---|---|
| Detection engine (Composer, npm, Go modules) | Working. 533 of 683 Recipes are reachable from a dependency |
...
| Conformance suites (`cauldron verify`) | Working. 9700 cases, 3448 of them checked against a live API |
...
That second line is the honest one. Of every Recipe: 9700 cases, 3448 run against
a live account and 6252 not. Documentation-derived cases are worth having,
and they are not the same as watching the provider do it. Adding a `verified:`
date to a case is a claim that someone did.

All 3448 are the cases whose provider can be asked without a key. Every other
...
| `undeclared` | No description is recorded for this Recipe. 476 of the 683, and falling -- see `cauldron discover` |
...
provider publishes nothing -- 476 of the 683 here -- is served exactly as it
...
The table reaches 533 of the 683 Recipes. The other 150 ship and can be named
