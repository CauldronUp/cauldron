...
| Area | State |
|---|---|
| Detection engine (Composer, npm, Go modules) | Working. 523 of 670 Recipes are reachable from a dependency |
...
| Conformance suites (`cauldron verify`) | Working. 9577 cases, 3388 of them checked against a live API |
...
That second line is the honest one. Of every Recipe: 9577 cases, 3388 run against
a live account and 6189 not. Documentation-derived cases are worth having,
and they are not the same as watching the provider do it. Adding a `verified:`
date to a case is a claim that someone did.

All 3388 are the cases whose provider can be asked without a key. Every other
...
| `undeclared` | No description is recorded for this Recipe. 463 of the 670, and falling -- see `cauldron discover` |
...
provider publishes nothing -- 463 of the 670 here -- is served exactly as it
...
The table reaches 523 of the 670 Recipes. The other 147 ship and can be named
