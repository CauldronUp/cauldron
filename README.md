...
| Area | State |
|---|---|
| Detection engine (Composer, npm, Go modules) | Working. DETECT_REACHABLE of 609 Recipes are reachable from a dependency |
...
| Conformance suites (`cauldron verify`) | Working. 6064 cases, 2845 of them checked against a live API |
...
That second line is the honest one. Of every Recipe: 6064 cases, 2845 run against
a live account and 3219 not. Documentation-derived cases are worth having,
and they are not the same as watching the provider do it. Adding a `verified:`
date to a case is a claim that someone did.

All 2845 are the cases whose provider can be asked without a key. Six are
...
Eleven are Deezer's, where **every failure is HTTP 200.** A record that does not
...
Seven are iNaturalist's, where **one field carries two schemes and the only
...
Nine are RCSB's, where **two of its fields are the letters
...
Eight are Europe PMC's, where **three failures have three shapes and the HTTP
...
Eight are OpenAIRE's, where **the JSON is XML wearing a costume.** Every text
...
Seven are ROR's, where **a path that does not exist says your authentication is
...
Eight are the Nobel Prize's, where **a laureate who does not exist is an array of
...
Eight are PDBe's, where **the response is keyed by what you asked for, and not as
...
Eight are Jisho's, where **there is no failure at all.** A search that finds
...
Eight are Stack Exchange's, where **every failure is 400 and one of them carries
...
Six are Kraken's, where **every success carries an empty error array, and an
...
Nine are Scryfall's, where **the prices are five nulls and one string.** Six
...
Seven are Lichess's, where **the game counts do not add up.** `count` reports
...
Eight are Gutendex's, where **a filter it cannot parse is dropped, and you get
...
Six are Wikimedia's, where **the error message is inside a map keyed
...
Six are SEC EDGAR's, where **the same field is a string in one endpoint and an
...
Nine are Alpha Vantage's, where **every failure is an HTTP 200.**
...
Sixteen are NCBI Entrez's, where **a `Content-Type` of
...
Eleven are Semantic Scholar's, where **a paper's own identifier cannot be routed
...
Fifteen are Mercado Libre's, where **most of the API is a 403 before it is
...
Thirteen are US Census's, where **the key gate fires before anything is
...
Ten are NASA's, where **one host and one key front three different APIs.**
...
Nine are Steam's, where **a missing app is a 200 that says success is false.**
...
Eleven are NOAA's, where **an unknown station is a 200 with an empty
...
Fourteen are the Guardian's, where **a parameter that looks like a filter adds an
...
Thirteen are arXiv's, which **answers Atom XML and nothing else** -- not with
...
Twelve are Healthchecks', where **the failure the service exists to detect
...
Fourteen are PubNub's, where **the cursor the whole API runs on is a bare number
...
Nine are Paystack's, and they are here for a **negative** result. Every response
...
Eighteen are Hugging Face's, where **a model that does not exist is a 401.**
...
Twelve are Vultr's, where **the total is the page size plus one.**
...
Thirteen are Linode's, and they are here to be read beside Vultr's. Two
...
Twelve are Scaleway's, where **a path that does not exist answers 401.** The
...
Ten are Status.io's, where **a malformed id and an unknown one are the same
...
Eight are UptimeRobot's, where **a wrong method answers 200.**
...
Six are Smartsheet's, where **four routes disagree about what a missing
...
Nine are Tink's, which **answers 401 in five different shapes on one host** --
...
Eight are OpenAQ's, where **the retired version is the one that still answers.**
...
One is Duffel's, which **refuses you for not naming a version before it checks
...
Two are FlightAware's, where **every mistake is the same 401** -- valid endpoint,
typo, malformed id and the bare root, byte-identical including the leading
newline and eight spaces of padding.

Three are Honeycomb's, where **one endpoint answers `problem+json` and six do
...
Five are New Relic's, **whose own description would generate a broken client.**
...
Four are Wise's, where **one endpoint answers without a credential and the rest
...
Four are Flutterwave's, and they exist to be read against Paystack's. Both wrap
...
Three are Giphy's, where **a failure arrives with an empty results array.** A
...
Four are Unsplash's, which **names its rate-limit headers without sending
...
Three are NewsAPI's, a **fifth spelling of the status inside the body** --
...
Three are GNews's, which **tells you whether your key is the right shape before
...
Three are Coda's, **whose failure states one fact four times** -- the status
...
One is GitBook's, which **checks whether a route exists before who you are** --
...
Seven are Split's, whose **failure sentence echoes the last five characters of
...
Five are Flagsmith's, which **answers three failures in three formats**: a
printed Python tuple served as JSON, a zero-byte 404, and a bare array holding
one object. No field in common.

Four are Brevo's. Three are Mailjet's. Three are Lithic's. Three are Moov's.
...
Three are Descope's, **whose two commonest error codes are undocumented.**
...
Eight are FRED's, which **says three different things about a bad key**: absent,
...
Six are Twelve Data's, written to test whether it repeats Alpha Vantage's
...
Five are Mapbox's, where **longitude comes first and the response says so** --
...
Six are OpenRouteService's, **whose own source comment gets that order
...
Three are Plivo's, **whose failure its own SDK cannot parse** -- a 401 in
...
Two are Sinch's, **whose documented API host has no public address**:
...
Thirteen are Cronitor's, **whose success declares JSON and sends nothing** --
...
Five are Checkly's, where **one run disagrees with itself by location** -- one
...
Six are ScrapingBee's, where **the target's status survives in a header until
...
Six are Browserless's, which **has no field for the target's status at all** --
...
Six are Brave Search's, which **calls a missing credential a validation
...
Seven are Serper's, **whose `statusCode` field only repeats the status line** --
...
Six are Temporal's, where **a create never returns the thing it created**.
...
Five are Inngest's, **whose failures declare `text/plain` and send JSON.** A
...
Four are Infobip's, which completes a five-way comparison the SMS Recipes have
...
Six are Customer.io's, **whose API answers its marketing site's 404** -- a 3817
...
Five are Make's, where **a token from the wrong region looks like a token that
...
Six are Turso's, which **answers a missing header with a JWT parsing error** --
...
Nine are SingleStore's, **whose create hands back a password and nothing else
...
Five are Airbyte's, which **does not call a partial failure a success**:
...
Five are Census's, where **whether you meet the credential check depends on the
...
Three are Rootly's, **whose severity is a second JSON:API document inside a
field** -- a complete `data`/`id`/`type`/`attributes` object as an attribute's
value, rather than a reference with the body in `included`.

Six are FireHydrant's, which **says which credential problem you have**, in two
...
Two are Mezmo's, **whose two surfaces check things in opposite orders** --
ingest resolves the credential first, management resolves the route first.

Three are Raygun's, which **announces JSON and sends fifteen bytes of prose**:
`Invalid API key`, no quotes, no braces, under `Content-Type: application/json`.

Four are AppSignal's, which **draws no line between an occurrence and a group**
-- the abstraction Rollbar, Bugsnag and Raygun all treat as central is simply
absent from its public REST API.

Two are Ory's, whose hosted playground **answers one 404 to everything** -- the
...
Seven are SuperTokens', which **answers 200 to a session that does not exist**:
...
Three are Wasabi's, whose S3 endpoint **is a marketing redirect until you try to
...
Three are Filebase's, whose **opaque trace header names the pod that served
you**. `X-Amz-Id-2` took two values across a dozen requests, `czNndy0x` and
`czNndy0y`, which are base64 for `s3gw-1` and `s3gw-2`.

Five are Backblaze's, which **serves one bucket under two protocols that
...
Five are HERE's, where **the path is checked before the key that is not there**.
...
Four are TomTom's, whose **Content-Type says text and whose body is JSON**, on
both a genuine RFC 7807 404 and a 401 that is nested under `detailedError` and is
not.

Five are what3words', whose **two credential failures come from two machines**.
...
Six are Frontegg's, which **names the host you got wrong**: a fabricated
...
Seven are Kinde's, whose **management API is rich and whose token endpoint is ten
bytes**. Three distinguishable credential sentences on one, and `not_found` in
plain text on the other.

Five are PropelAuth's, whose **status code tracks whether you sent a header, not
...
Six are Fireworks', which **quotes back a path the caller never sent**: the
...
Six are Rippling's, whose **renamed surface did not move**: `/platform/api/workers`
-- the term its own current documentation uses -- 404s, while
`/platform/api/employees` still answers 401.

Five are Workday's, whose gateway **tells anybody which services exist**. A
...
Five are Diffbot's, whose own client has **a class named for succeeding and
...
Five are Bright Data's, which **found a bug in this project's credential check**.
...
Five are Zyte's, which **draws the line the other two blur**: a target that
...
Nine are Koyeb's, whose create **points at three deployments at once**:
active, latest and last-provisioned, two of which routinely name different
records with nothing saying which to poll.

Eight are Northflank's, which **validates the body before the credential** on its
...
Nine are Porter's, which **has no HTTP API to model**. `api.porter.run` is dead,
...
Eight are Expensify's, whose **HTTP status is always 200**. The real one is a body
field reusing HTTP numbers with none of their meaning: a genuine authentication
failure answers `responseCode: 404`.

Ten are Sage Intacct's, whose gateway **cannot tell three mistakes apart** -- an
...
Eight are ClickHouse's, whose **API root is its own description**: `GET /v1`
...
Six are Timescale's, found after the search for it. `console.timescale.com` is
dead DNS and `api.timescale.com` is **a load balancer that refuses TLS** -- worse
than dead, because a client will keep retrying it.

Three are InfluxDB's, whose Cloud host **does not serve InfluxDB**.
...
Six are ShipBob's, and every one of them is a failure, because **the host has been
...
Five are Onfleet's, which has **no 404 reachable anywhere**: every unrouted path
...
Six are Payoneer's, where **a payment is cancelled by doing nothing**: a debit
...
Five are Treasury Prime's, where **cancelling is a status you write**, not an
endpoint you call: a PATCH setting status to canceled, valid only while pending.

Eleven are Melio's, where a cancel route is **proven to exist without ever being
...
Four are Tipalti's, whose payee is **provisional from birth** -- isPayable false
in its very first create response, and read-only on every model that could set it.

Five are Anrok's, whose two calculate endpoints **differ by one absent field**:
a stored transaction carries `"version": 1` and an ephemeral quote does not.

Five are TriNet's, whose gateway answers from **two different subsystems** for an
empty credential and a wrong one -- different error-code namespaces, and
different capitalisation of the sentence beside them.

Five are Remote.com's, which **says twice who employs somebody**: `type` and
`employment_model` are both on every record and neither is derivable from the
other.

Four are Zenvia's, whose send returns an id **nothing takes back** -- its own
description has no GET for a message or a batch anywhere.

Four are Kaleyra's, where **one identifier has three names**: a parent id, that
id with `:1` appended, and the same value called campaign_id on the status route.

Four are Textline's, which has **no delivery-status vocabulary at all** -- a
Post carries only human triage fields, never anything about carriage.

Ten are Constant Contact's, whose **second create route sets consent**: one door
refuses a duplicate with a 409 and the other marks permission explicit with no
exception for somebody who unsubscribed.

Six are Drip's, whose documentation **contradicts itself on the same page** about
whether an inactive person can be modified.

Five are Omnisend's, whose ordinary create is an upsert -- "Existing contact
updated (upsert by email)" -- with **no guard-rail language anywhere**.

Four are Duo's, whose decision **does not exist when the call returns**: an
asynchronous authentication hands back a txid and the verdict appears a poll
later.

Five are Castle's, whose **edge tier crashes on a header with no whitespace** --
500, and an openresty HTML page where every other failure is JSON.

Ten are Sardine's, whose **decision shape is sales-gated**, so what a risk API
returns could not be established at all -- and whose refusal echoes a mistyped
client id back.

Seven are MinIO's, which **never examines the name you asked about**: its own
default root username and an invented one answer byte-identically.

Seven are Nile's, where **existence is unknowable before you authenticate** -- one
empty 401 whether the database and tenant are real or invented.

Five are Tigris's, which **echoes your bucket name whatever it is**, so the
credential verdict is the only thing that varies.

Nine are Elastic Cloud's, where **attaching any credential hides the routing**:
with none, a 404 and a 405 answer properly; with garbage, both collapse into one
undocumented shape.

Fifteen are Optimizely's, whose **assignment is computed on the client** --
MurmurHash3, seed 1, from its own open-source SDK -- so there is no decide
endpoint to ask.

Thirteen are VWO's, whose **routing failure wears a success status**: any
unsupported API version answers HTTP 200 with the error in the body.

Five are Flexport's, whose container carries **four date pairs and no status** --
the status enum lives one level up, on the shipment.

Five are Bringg's, whose public API **leaks an untranslated i18n key**: Rails'
own placeholder for a string nobody wrote, in a live error field.

Eight are Radar's, which **tells you to fetch the wrong kind of key** -- asking
for the publishable one on an endpoint documented as needing the secret.

Five are TalkJS's, whose conversation **exists and cannot be seen**: created by a
caller-chosen PUT, and invisible until somebody sends a message.

Five are CometChat's, whose conversation **has no create route at all** -- it is a
computed view of a group, and exists the moment the group does.

Six are Whereby's, where **the URL and the room expire together**, which is why
the room URL is unsigned where PDFMonkey's and Api2Pdf's are not.

Five are Clio's, whose **two 401s share no field names** -- `error` is an object
in one and a bare string in the other.

Five are Aha's, whose **custom-field schema disagrees with its own values** about
what type a field is, and mints two ids for one field name on two record types.

Five are Adapty's, whose **documented credential scheme is not the live one**:
`Api-Key` cannot be told from sending nothing, and the undocumented Bearer scheme
turns out to be the one its gateway parses.

Seven are JustCall's, whose recording link has **three shapes and no lifetime**
across its own three documents.

Four are CloudTalk's, whose **two hosts disagree about their own API** on both
credential granularity and routing order.

Five are Missive's, whose attachment URL is **signed and undated** -- landing
between PDFMonkey's stated hour and Api2Pdf's stated day.

Four are Papaya Global's, which is **not the product its name promises**: the
...
Eight are Ayrshare's, whose **partial failure has no word of its own**: a boolean
status, so a mostly-successful batch and a total failure read alike.

Seven are Cognito's, where **the wrong region is the same as no pool** -- the
identical sentence, differing only in the id echoed back, for an id whose first
half names the region.

Five are Google Maps', whose **status line is honest about routing and silent
...
Seven are Livepeer's, whose asset and task **disagree about being done** -- two
status vocabularies sharing two words, where the task's `completed` says nothing
about the asset's `ready`.

Five are UPS's, whose **token endpoint tells the truth and whose API does not**:
two codes separate an absent credential from a wrong one before you have a
token, and one body covers both once you do.

Eight are USPS's, where **a token is judged in three stages** -- absent and
non-JWT collapse together, and a JWT signed wrongly gets its own sentence.

Eight are Canada Post's, which **checks the parcel's shape before the
caller's**: twelve, thirteen or sixteen alphanumerics, validated ahead of the
credential and even against a wrong one.

Four are Reddit's, which is **mostly a wall and says so** -- every post endpoint
blocked on all three hosts, so the identifier scheme is cited and never
fixtured.

Thirteen are Bluesky's, where **the handle is a pointer and the DID is the
identity**: the same profile fetched both ways comes back byte-identical.

Nine are Mastodon's, whose **id is local and whose uri is not**, and whose
reference and wire disagree about a missing status on the same server, the same
day.

Fourteen are Coinbase's, whose **two live APIs disagree about staleness** -- one
carries a nanosecond timestamp and a sequence, the other carries neither and
puts empty strings where prices go.

Ten are Binance's, where **three price endpoints prove three different things**
and only one of them lets a caller detect a gap. Its book never names the symbol
it was asked for.

Six are Alchemy's, where **the credential is checked before the route exists** --
the key is a path segment, so there is no such thing as an unrouted path.

Nine are WeatherAPI's, whose **own field table contradicts itself**: three fields
described as local time in Unix time, which is not a thing.

Nine are Tomorrow.io's, which **has one clock and it is not the right one** -- a
clean UTC observation time, and nothing at all about when the forecast was
computed.

Eight are Visual Crossing's, **the one that says what the call cost**, and the
only true 405 in its group. It advertises POST in an Allow header and crashes on
it.

Eight are IPinfo's, where **no credential is not a failure**: an absent key is a
200 with a reduced body and a wrong one is a 403.

Eight are MaxMind's, whose **credential stands in front of the question** -- a
reserved address never reaches the reserved-address error its own documentation
declares.

Seven are Geoapify's, which **has no confidence to report** at any tier, and
whose credential check runs before routing on every path and verb tried.

Five are Clearbit's, which **is not retired and cannot be read**: the hosts still
answer and still stamp a 2019 version header, and the documentation now
redirects to a login.

Three are Hunter.io's, whose **two limits are the wrong way round** -- 403 is the
rate limit and 429 is the plan quota.

Four are Apollo.io's, where **the absent credential is a validation failure**:
422 for nothing sent, 401 for something wrong.

Eight are Daily's, whose **expired room becomes a zombie** -- its own word, for a
room that is not deleted and does not exist.

Eleven are LiveKit's, where **a bad credential outranks routing and an absent one
does not**, so sending nothing tells you more than sending something wrong.

Nine are Agora's, whose **collapse runs the other way** from its neighbour's:
five broken shapes fold into rejected, and only a truly absent header stands
apart.

Ten are Discogs's, where **a merged release leaves no trace** -- no redirect, no
tombstone, and a sentence that declines to choose between never and no longer.

Five are Last.fm's, and **the one place two schemes name one thing**: an artist
resolves identically by name and by MusicBrainz identifier.

Seven are Genius's, where **two pieces of software refuse the caller** and only
one of them agrees with the other about whether routing matters.

Seven are CockroachDB's, whose **state enum has no word for deleting**, so a
cluster mid-teardown is either still created or already gone.

Seven are Redis Cloud's, where **half a credential is worse than none** -- either
header alone answers a bare nginx 500 and only both together reach a real 401.

Five are SurrealDB's, which **answers the deleting question twice, differently**:
no enum on the instance, a six-value one on the organisation, with the second
value spelled wrong.

Five are Checkout.com's, **whose refusal is zero bytes** -- no body, no
Content-Type, across four credential shapes and five methods.

Four are Klarna's, which **tells you not to trust its own status field**, in a
sentence from its own reference.

Five are Mangopay's, which **blocks a replayed idempotency key for a day**
whatever the first call did, where Checkout.com forgets it and Stripe replays the
failure.

Eight are Kiwi's, **an API that answers behind documentation that does not** --
alive and probeable, with a docs portal wanting an account at the vendor's own
domain.

Eight are Hotelbeds's, where **the signature's quality cannot be observed**: the
key splits cleanly and every signature shape collapses into one 403.

Six are Veriff's, which **never looks at the signature until the id is real** --
three credential codes from the client identifier alone, each echoing what it was
given.

Seven are Alloy's, **whose refusals split across two statuses**: unreadable is a
400 and readable-but-wrong is a 401.

Six are Anthropic's, where **one route resolves the id before the credential**
and its sibling on the same host does not.

Six are Stability AI's, which **hands the wrong key back masked** -- and the mask
is exactly as long as the value it hides.

Five are Fal.ai's, whose **queue position vanishes rather than reaching zero**,
so a caller treating a missing number as zero reports next-up for something
already running.

Five are MX's, which **has three fields for one question and no answer**: the one
that would say a connection needs re-authentication is never true in any
published example.

Five are Belvo's, **the one that names the re-authentication state** outright,
where its neighbours circle it.

Five are Basiq's, where **the connection is fine and the account is not** -- two
accounts under one active connection, one of them two years stale, with nothing
on the connection saying so.

Nine are DeepL's, where **one wrong key gets two different sentences** -- the
...
Nine are Moodle's, where **every failure is a success with an exception inside**
-- nine ways to fail, including an invented operation name, all answering 200.

Seven are Clever's, whose **Allow header is spelled one letter per header**:
nine headers named zero through eight, each holding one character.

Ten are OpenSky's, whose **receiver field is always null for anyone anonymous**
-- the one provider in its group with a field for provenance, reserved.

Six are AviationStack's, whose **own published schema does not match its wire**:
four declared fields, none of which the live error carries.

Ten are AeroDataBox's, where **rejected and unentitled collapse at the gateway**
-- once the host header names a real product, a garbage key and a wrong-plan key
answer alike.

Five are Nutritionix's, where **half a credential is the same as none**, and
whose nutrient array carries a number and a value with no unit and no name.

Five are Edamam's, which gives **every nutrient a unit and the total no basis**:
a whole-recipe figure with nothing saying it needs dividing.

Seven are Spoonacular's, which **says per-serving everywhere but on the wire** --
the clearest statement of the group, in the documentation rather than the body.

Five are Google Books', where **sending a key fails faster than sending none**:
any key skips the exhausted anonymous quota and fails on a per-project check.

Eight are ISBNdb's, whose **method rejection leaks its own internal route** --
a path segment that appears in no documentation and in nothing that was sent.

Six are Hardcover's, which **separates the work from the edition and says so**,
in prose and structurally, with the ISBN fields only on editions.

Eight are Toggl's, and **the negative duration nobody can confirm**: the sign is
still documented and the formula that made it mean something is gone.

Four are Harvest's, whose **absent credential is disguised as a missing route**,
and whose 404 splits on routing underneath while any credential collapses it.

Four are Clockify's, where **every region is the same region** -- four
documented hosts and an invented one all answer identically.

Twelve are Mixpanel's, whose **silent acceptance is now asserted rather than
described**, and whose gate turns out to be the presence of three fields and
never their validity.

Six are Amplitude's, **the only one of three that checks the key on ingest**: an
unrecognised one is refused by name.

Four are Heap's, where **the application identifier authenticates nothing** -- one
never registered gets the identical success a real one would.

Eight are Exa's, **the one search API that labels a result cached**, per
document, where its two neighbours label nothing.

Six are Tavily's, **whose answer cites nothing**: no citation array, no result
index, established by walking the schema rather than the prose.

Eight are SerpApi's, which **times its own scrape and not the page** -- two
clocks about the request and none about the result.

Eleven are Fastly's, where **locked and active are independent and only one is
provable**: no response anywhere shows a version active.

Five are KeyCDN's, **the clean no to its group's question** -- purge answers in
the past tense with nothing to poll.

Eight are Namecheap's, whose **credential includes the caller's own address**, and
where a valid address that is not the whitelisted one reads as a wrong key.

Ten are Porkbun's, which **takes its credential in a header after all**, against
every example it publishes, and answers 400 to every credential failure.

Nine are DNSimple's, where **an unmentioned scheme crashes the server** -- Basic,
which its documentation never names, answers 500 where everything else is 401.

Eight are OpenCorporates', **an aggregator that has closed**, whose own
documentation says 401 means incorrect credentials and whose 401 fires for
absent ones too.

Twelve are Companies House's, where **the ignored password is refused**: empty
gets 401, non-empty gets 400, for a field the guide says is not read.

Six are PatentsView's, **for an API that no longer exists** -- every host
redirects, and the one the credential lived on is gone from DNS.

Five are Cloudflare Stream's, whose **own example contradicts its own field**: a
video in progress carrying an error reason code.

Seven are api.video's, which has **no single answer to whether a video is
ready** -- three vocabularies, one of which has no failure value at all.

Six are Bunny Stream's, where **the library id's shape is checked and its owner
is not**: numeric ids are indistinguishable, non-numeric ones are refused before
the credential.

Nine are Bitly's, which **calls retargeting redirecting and bills for it** --
a first-class operation charged as an encode, with no field anywhere holding the
destination a link used to have.

Nine are Dub's, where **a bad key is not rejected until a workspace resolves**:
the same wrong token answers a missing-workspace 404 without one and an
invalid-key 401 with one.

Ten are Short.io's, **one fact in four envelopes** -- the same unauthorized
condition rendered four different ways depending only on which route was asked.

Five are SurveyMonkey's, whose **own example gets its own wire wrong**, quoting
a numeric error id as a string, and whose responses carry identifiers and never
a label.

Six are Jotform's, whose **envelope and status can disagree**, and whose
not-found echoes the path parameter's name rather than the value it was given.

Eight are Tally's, where **every failure is the same twelve bytes** -- absent,
...
Ten are Bitwarden's, whose **two refusals differ only in a header clients
ignore**: both 401 with zero bytes, separated by the challenge alone.

Eight are Doppler's, which **refuses before it routes, without exception**, so
it never tells a caller their path was wrong.

Seven are Better Stack's, whose **404 always says GET** whatever method was
sent, verified across six verbs.

Six are Instatus', where **a monitor can be up and paused at once** -- two
independent fields, and no last-checked time on the record at all.

Six are Pingdom's, which **separates the last poll from the last outage**, the
only provider in its group to do so, and the only one with a state for
unconfirmed.

Five are Transitland's, whose **two credential failures differ only by a
header**, and which dates every source feed separately -- the only honest answer
available to an aggregator.

Seven are Navitia's, **three credential sentences and no workaround**, where an
invented coverage region answers exactly what a real one would.

Five are 511's, where **a dot in the path skips the credential entirely**: any
segment carrying a file extension is served by the web server before the
application is reached.

Eight are OMDb's, and **the success-shaped failure that no longer is** -- the
reputation was real and is out of date, though the envelope still holds the
string False rather than a boolean.

Eight are Trakt's, where **the credential is the only thing that ever speaks**:
with no key header the edge answers instead of the application, and the version
header never distinguishes anything.

Eight are Watchmode's, whose **description guarantees nothing at all** -- an
...
| `undeclared` | No description is recorded for this Recipe. 463 of the 576, and falling -- see `cauldron discover` |
...
provider publishes nothing -- 463 of the 576 here -- is served exactly as it
...
The table reaches 439 of the 576 Recipes. The other 137 ship and can be named
