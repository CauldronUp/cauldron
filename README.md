...
| Area | State |
|---|---|
| Detection engine (Composer, npm, Go modules) | Working. 317 of 364 Recipes are reachable from a dependency |
...
| Conformance suites (`cauldron verify`) | Working. 3600 cases, 1137 of them checked against a live API |
| Spec drift (`cauldron drift`) | Working. 44 Recipes are checked against their provider's own OpenAPI document; 308 name none yet |
...
That second line is the honest one. Of every Recipe: 3600 cases, 1137 run against
a live account and 2463 not. Documentation-derived cases are worth having,
...
**The number will not reach 364.** Most providers publish nothing, and this
...
44 unchanged, 0 moved, 0 unreachable, 0 in a format this cannot read, 0 unrecorded, 308 with no description to check.
...
| `undeclared` | No description is recorded for this Recipe. 317 of the 364, and falling -- see `cauldron discover` |
...
provider publishes nothing -- 317 of the 364 here -- is served exactly as it
...
The table reaches 317 of the 364 Recipes. The other 47 ship and can be named
