...
| Area | State |
|---|---|
| Detection engine (Composer, npm, Go modules) | Working. 490 of 635 Recipes are reachable from a dependency |
...
| Conformance suites (`cauldron verify`) | Working. 9197 cases, 3314 of them checked against a live API |
...
That second line is the honest one. Of every Recipe: 9197 cases, 3314 run against
a live account and 5883 not. Documentation-derived cases are worth having,
and they are not the same as watching the provider do it. Adding a `verified:`
date to a case is a claim that someone did.

All 3314 are the cases whose provider can be asked without a key. Every other
...
**The number will not reach 635.** Most providers publish nothing, and this
...
| `undeclared` | No description is recorded for this Recipe. 501 of the 635, and falling -- see `cauldron discover` |
...
provider publishes nothing -- 501 of the 635 here -- is served exactly as it
...
The table reaches 490 of the 635 Recipes. The other 145 ship and can be named
