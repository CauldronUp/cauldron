...
| Webhooks (lifecycle events, signing, delivery) | Working. Payload envelopes are declarable per Recipe. Of the 99 Recipes that emit events, 10 declare one and 89 fall back to a default modelled on Stripe's and not equal to it -- Stripe's own Recipe now declares the fuller real thing -- so the fallback is a convention rather than a claim about any provider |
...
| Conformance suites (`cauldron verify`) | Working. 1711 cases, 51 of them checked against a live API |
...
That second line is the honest one. Of every Recipe: 1711 cases, 51 run against
