...
| Area | State |
|---|---|
| Detection engine (Composer, npm, Go modules) | Working. 279 of 309 Recipes are reachable from a dependency |
...
| Conformance suites (`cauldron verify`) | Working. 3073 cases, 758 of them checked against a live API |
| Spec drift (`cauldron drift`) | Working. 11 Recipes are checked against their provider's own OpenAPI document, 1 provider publishes one this cannot read, and 297 publish none |
...
That second line is the honest one. Of every Recipe: 3073 cases, 758 run against
...
The seven hundred and fifty-eight are the cases whose provider can be asked without a key. Six are
...
| `undeclared` | The provider publishes nothing to check. 297 of the 309 |
...
The table reaches 279 of the 309 Recipes. The other 30 ship and can be named
