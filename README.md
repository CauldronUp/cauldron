...
| Area | State |
|---|---|
| Detection engine (Composer, npm, Go modules) | Working. 522 of 669 Recipes are reachable from a dependency |
...
| Conformance suites (`cauldron verify`) | Working. 9567 cases, 3383 of them checked against a live API |
...
That second line is the honest one. Of every Recipe: 9567 cases, 3383 run against
a live account and 6184 not. Documentation-derived cases are worth having,
and they are not the same as watching the provider do it. Adding a `verified:`
date to a case is a claim that someone did.

All 3383 are the cases whose provider can be asked without a key. Every other
...
| `undeclared` | No description is recorded for this Recipe. 463 of the 669, and falling -- see `cauldron discover` |
...
provider publishes nothing -- 463 of the 669 here -- is served exactly as it
...
The table reaches 522 of the 669 Recipes. The other 147 ship and can be named
