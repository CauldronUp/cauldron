...
| Area | State |
|---|---|
| Detection engine (Composer, npm, Go modules) | Working. 491 of 636 Recipes are reachable from a dependency |
...
| Conformance suites (`cauldron verify`) | Working. 9208 cases, 3322 of them checked against a live API |
...
That second line is the honest one. Of every Recipe: 9208 cases, 3322 run against
a live account and 5886 not. Documentation-derived cases are worth having,
and they are not the same as watching the provider do it. Adding a `verified:`
date to a case is a claim that someone did.

All 3322 are the cases whose provider can be asked without a key. Every other
...
**The number will not reach 636.** Most providers publish nothing, and this
...
| `undeclared` | No description is recorded for this Recipe. 501 of the 636, and falling -- see `cauldron discover` |
...
provider publishes nothing -- 501 of the 636 here -- is served exactly as it
...
The table reaches 491 of the 636 Recipes. The other 145 ship and can be named
