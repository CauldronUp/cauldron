...
| Area | State |
|---|---|
| Detection engine (Composer, npm, Go modules) | Working. 527 of 677 Recipes are reachable from a dependency |
...
| Conformance suites (`cauldron verify`) | Working. 9643 cases, 3427 of them checked against a live API |
...
That second line is the honest one. Of every Recipe: 9643 cases, 3427 run against
a live account and 6216 not. Documentation-derived cases are worth having,
and they are not the same as watching the provider do it. Adding a `verified:`
date to a case is a claim that someone did.

All 3427 are the cases whose provider can be asked without a key. Every other
...
| `undeclared` | No description is recorded for this Recipe. 470 of the 677, and falling -- see `cauldron discover` |
...
provider publishes nothing -- 470 of the 677 here -- is served exactly as it
...
The table reaches 527 of the 677 Recipes. The other 150 ship and can be named
