...
| Area | State |
|---|---|
| Detection engine (Composer, npm, Go modules) | Working. 493 of 638 Recipes are reachable from a dependency |
...
| Conformance suites (`cauldron verify`) | Working. 9234 cases, 3336 of them checked against a live API |
...
ok    docker                 running
warn  ports                  6389 wanted by redis, held by other-project-redis-1
...
That second line is the honest one. Of every Recipe: 9234 cases, 3336 run against
a live account and 5898 not. Documentation-derived cases are worth having,
and they are not the same as watching the provider do it. Adding a `verified:`
date to a case is a claim that someone did.

All 3336 are the cases whose provider can be asked without a key. Every other
...
**The number will not reach 638.** Most providers publish nothing, and this
...
| `undeclared` | No description is recorded for this Recipe. 501 of the 638, and falling -- see `cauldron discover` |
...
provider publishes nothing -- 501 of the 638 here -- is served exactly as it
...
The table reaches 493 of the 638 Recipes. The other 145 ship and can be named
