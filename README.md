...
| Area | State |
|---|---|
| Detection engine (Composer, npm, Go modules) | Working. 500 of 645 Recipes are reachable from a dependency |
...
| Conformance suites (`cauldron verify`) | Working. 9307 cases, 3342 of them checked against a live API |
...
ok    docker                 running
warn  ports                  6459 wanted by redis, held by other-project-redis-1
...
That second line is the honest one. Of every Recipe: 9307 cases, 3342 run against
a live account and 5965 not. Documentation-derived cases are worth having,
and they are not the same as watching the provider do it. Adding a `verified:`
date to a case is a claim that someone did.

All 3342 are the cases whose provider can be asked without a key. Every other
...
**The number will not reach 645.** Most providers publish nothing, and this
...
| `undeclared` | No description is recorded for this Recipe. 501 of the 645, and falling -- see `cauldron discover` |
...
provider publishes nothing -- 501 of the 645 here -- is served exactly as it
...
The table reaches 500 of the 645 Recipes. The other 145 ship and can be named
