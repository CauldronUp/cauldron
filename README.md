...
| Area | State |
|---|---|
| Detection engine (Composer, npm, Go modules) | Working. 533 of 820 Recipes are reachable from a dependency |
...
| Conformance suites (`cauldron verify`) | Working. 11069 cases, 4053 of them checked against a live API |
...
That second line is the honest one. Of every Recipe: 11069 cases, 4053 run against
a live account and 7016 not. Documentation-derived cases are worth having,
and they are not the same as watching the provider do it. Adding a `verified:`
date to a case is a claim that someone did.

All 4053 are the cases whose provider can be asked without a key. Every other
...
| `undeclared` | No description is recorded for this Recipe. 567 of the 820, and falling -- see `cauldron discover` |
...
provider publishes nothing -- 567 of the 820 here -- is served exactly as it
...
The table reaches 533 of the 820 Recipes. The other 287 ship and can be named
