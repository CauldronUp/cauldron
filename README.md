...
| Area | State |
|---|---|
| Detection engine (Composer, npm, Go modules) | Working. 487 of 632 Recipes are reachable from a dependency |
...
| Conformance suites (`cauldron verify`) | Working. 9168 cases, 3313 of them checked against a live API |
...
That second line is the honest one. Of every Recipe: 9168 cases, 3313 run against
a live account and 5855 not. Documentation-derived cases are worth having,
and they are not the same as watching the provider do it. Adding a `verified:`
date to a case is a claim that someone did.

All 3313 are the cases whose provider can be asked without a key. Every other
...
**The number will not reach 632.** Most providers publish nothing, and this
...
| `undeclared` | No description is recorded for this Recipe. 501 of the 632, and falling -- see `cauldron discover` |
...
provider publishes nothing -- 501 of the 632 here -- is served exactly as it
...
The table reaches 487 of the 632 Recipes. The other 145 ship and can be named
