...
| Conformance suites (`cauldron verify`) | Working. 1673 cases, 5 of them checked against a live API |
...
That second line is the honest one. Of every Recipe: 1673 cases, 5 run against
a live account and 1548 not. Documentation-derived cases are worth having,
and they are not the same as watching the provider do it. Adding a `verified:`
date to a case is a claim that someone did.

The five are OpenRouter's model-catalogue cases, whose endpoint is public and
needs no key, so the numbers in them were read from the provider rather than
inferred. Its completion cases carry no date, because calling that endpoint
costs money.
