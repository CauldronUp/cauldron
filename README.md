...
| Area | State |
|---|---|
| Detection engine (Composer, npm, Go modules) | Working. 181 of 203 Recipes are reachable from a dependency |
| Recipe format and validator | Working |
| Recipe runtime (routing, state, auth, pagination) | Working |
| Webhooks (lifecycle events, signing, delivery) | Working. Payload envelopes are declarable per Recipe. Of the 100 Recipes that emit events, 42 declare one and 58 fall back to a default modelled on Stripe's and not equal to it -- Stripe's own Recipe now declares the fuller real thing -- so the fallback is a convention rather than a claim about any provider |
...
| Conformance suites (`cauldron verify`) | Working. 2147 cases, 58 of them checked against a live API |
...
leaving something unwired. Of 484 events declared across these Recipes, 264 are
in that category.
...
That second line is the honest one. Of every Recipe: 2147 cases, 58 run against
a live account and 2089 not. Documentation-derived cases are worth having,
...
The table reaches 181 of the 203 Recipes. The other 22 ship and can be named
