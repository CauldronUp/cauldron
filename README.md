...
| Webhooks (lifecycle events, signing, delivery) | Working. Payload envelopes are declarable per Recipe. Of the 99 Recipes that emit events, 8 declare one and 91 fall back to Stripe's shape, which is a default rather than a claim about that provider |
...
| Conformance suites (`cauldron verify`) | Working. 1709 cases, 51 of them checked against a live API |
...
That second line is the honest one. Of every Recipe: 1709 cases, 51 run against
a live account and 1658 not. Documentation-derived cases are worth having,
