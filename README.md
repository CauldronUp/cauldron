...
| Area | State |
|---|---|
| Detection engine (Composer, npm, Go modules) | Working. 492 of 637 Recipes are reachable from a dependency |
...
| Conformance suites (`cauldron verify`) | Working. 9221 cases, 3324 of them checked against a live API |
...
That second line is the honest one. Of every Recipe: 9221 cases, 3324 run against
a live account and 5897 not. Documentation-derived cases are worth having,
and they are not the same as watching the provider do it. Adding a `verified:`
date to a case is a claim that someone did.

All 3324 are the cases whose provider can be asked without a key. Every other
...
**The number will not reach 637.** Most providers publish nothing, and this
...
| `undeclared` | No description is recorded for this Recipe. 501 of the 637, and falling -- see `cauldron discover` |
...
provider publishes nothing -- 501 of the 637 here -- is served exactly as it
...
The table reaches 492 of the 637 Recipes. The other 145 ship and can be named
