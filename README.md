...
| Area | State |
|---|---|
| Detection engine (Composer, npm, Go modules) | Working. 509 of 654 Recipes are reachable from a dependency |
...
| Conformance suites (`cauldron verify`) | Working. 9406 cases, 3348 of them checked against a live API |
...
That second line is the honest one. Of every Recipe: 9406 cases, 3348 run against
a live account and 6058 not. Documentation-derived cases are worth having,
and they are not the same as watching the provider do it. Adding a `verified:`
date to a case is a claim that someone did.

All 3348 are the cases whose provider can be asked without a key. Every other
...
| `undeclared` | No description is recorded for this Recipe. 509 of the 654, and falling -- see `cauldron discover` |
...
provider publishes nothing -- 509 of the 654 here -- is served exactly as it
...
The table reaches 509 of the 654 Recipes. The other 145 ship and can be named
