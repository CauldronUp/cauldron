...
| Area | State |
|---|---|
| Detection engine (Composer, npm, Go modules) | Working. 274 of 303 Recipes are reachable from a dependency |
...
| Conformance suites (`cauldron verify`) | Working. 3028 cases, 713 of them checked against a live API |
| Spec drift (`cauldron drift`) | Working. 11 Recipes are checked against their provider's own OpenAPI document, 1 provider publishes one this cannot read, and 291 publish none |
...
That second line is the honest one. Of every Recipe: 3028 cases, 713 run against
...
The seven hundred and thirteen are the cases whose provider can be asked without a key. Six are
...
| `undeclared` | The provider publishes nothing to check. 291 of the 303 |
...
The table reaches 274 of the 303 Recipes. The other 29 ship and can be named
