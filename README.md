...
| Area | State |
|---|---|
| Detection engine (Composer, npm, Go modules) | Working. 272 of 299 Recipes are reachable from a dependency |
...
| Conformance suites (`cauldron verify`) | Working. 2996 cases, 681 of them checked against a live API |
| Spec drift (`cauldron drift`) | Working. 11 Recipes are checked against their provider's own OpenAPI document, 1 provider publishes one this cannot read, and 287 publish none |
...
That second line is the honest one. Of every Recipe: 2996 cases, 681 run against
...
The six hundred and eighty-one are the cases whose provider can be asked without a key. Six are
...
| `undeclared` | The provider publishes nothing to check. 287 of the 299 |
...
The table reaches 272 of the 299 Recipes. The other 27 ship and can be named
