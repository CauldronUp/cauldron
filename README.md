...
| Webhooks (lifecycle events, signing, delivery) | Working. Payload envelopes are declarable per Recipe. Of the 99 Recipes that emit events, 25 declare one and 74 fall back to a default modelled on Stripe's and not equal to it -- Stripe's own Recipe now declares the fuller real thing -- so the fallback is a convention rather than a claim about any provider |
...
| Conformance suites (`cauldron verify`) | Working. 1725 cases, 51 of them checked against a live API |
...
That second line is the honest one. Of every Recipe: 1725 cases, 51 run against
a live account and 1674 not. Documentation-derived cases are worth having,
