...
| Area | State |
|---|---|
| Detection engine (Composer, npm, Go modules) | Working. 524 of 671 Recipes are reachable from a dependency |
...
| Conformance suites (`cauldron verify`) | Working. 9588 cases, 3392 of them checked against a live API |
...
That second line is the honest one. Of every Recipe: 9588 cases, 3392 run against
a live account and 6196 not. Documentation-derived cases are worth having,
and they are not the same as watching the provider do it. Adding a `verified:`
date to a case is a claim that someone did.

All 3392 are the cases whose provider can be asked without a key. Every other
...
| `undeclared` | No description is recorded for this Recipe. 464 of the 671, and falling -- see `cauldron discover` |
...
provider publishes nothing -- 464 of the 671 here -- is served exactly as it
...
The table reaches 524 of the 671 Recipes. The other 147 ship and can be named
