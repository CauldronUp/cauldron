...
| Area | State |
|---|---|
| Detection engine (Composer, npm, Go modules) | Working. 506 of 651 Recipes are reachable from a dependency |
...
| Conformance suites (`cauldron verify`) | Working. 9375 cases, 3346 of them checked against a live API |
...
That second line is the honest one. Of every Recipe: 9375 cases, 3346 run against
a live account and 6029 not. Documentation-derived cases are worth having,
and they are not the same as watching the provider do it. Adding a `verified:`
date to a case is a claim that someone did.

All 3346 are the cases whose provider can be asked without a key. Every other
...
| `undeclared` | No description is recorded for this Recipe. 506 of the 651, and falling -- see `cauldron discover` |
...
provider publishes nothing -- 506 of the 651 here -- is served exactly as it
...
The table reaches 506 of the 651 Recipes. The other 145 ship and can be named
