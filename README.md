...
| Area | State |
|---|---|
| Detection engine (Composer, npm, Go modules) | Working. 283 of 326 Recipes are reachable from a dependency |
...
| Conformance suites (`cauldron verify`) | Working. 3252 cases, 937 of them checked against a live API |
| Spec drift (`cauldron drift`) | Working. 33 Recipes are checked against their provider's own OpenAPI document; 293 name none yet |
...
That second line is the honest one. Of every Recipe: 3252 cases, 937 run against
...
**The number will not reach 326.** Most providers publish nothing, and this
...
33 unchanged, 0 moved, 0 unreachable, 0 in a format this cannot read, 0 unrecorded, 293 with no description to check.
...
| `undeclared` | No description is recorded for this Recipe. 293 of the 326, and falling -- see `cauldron discover` |
...
provider publishes nothing -- 293 of the 326 here -- is served exactly as it
...
The table reaches 283 of the 326 Recipes. The other 43 ship and can be named
