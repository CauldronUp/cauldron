...
| Area | State |
|---|---|
| Detection engine (Composer, npm, Go modules) | Working. 533 of 740 Recipes are reachable from a dependency |
...
| Conformance suites (`cauldron verify`) | Working. 10311 cases, 3734 of them checked against a live API |
...
That second line is the honest one. Of every Recipe: 10311 cases, 3734 run against
a live account and 6577 not. Documentation-derived cases are worth having,
and they are not the same as watching the provider do it. Adding a `verified:`
date to a case is a claim that someone did.

All 3734 are the cases whose provider can be asked without a key. Every other
...
| `undeclared` | No description is recorded for this Recipe. 524 of the 740, and falling -- see `cauldron discover` |
...
provider publishes nothing -- 524 of the 740 here -- is served exactly as it
...
The table reaches 533 of the 740 Recipes. The other 207 ship and can be named
