...
| Area | State |
|---|---|
| Detection engine (Composer, npm, Go modules) | Working. 252 of 276 Recipes are reachable from a dependency |
...
| Conformance suites (`cauldron verify`) | Working. 2817 cases, 502 of them checked against a live API |
...
That second line is the honest one. Of every Recipe: 2817 cases, 502 run against
...
The five hundred and two are the cases whose provider can be asked without a key. Six are
...
on an endpoint where `craft` is not always `"ISS"` because Tiangong is in there
too.
...
The table reaches 252 of the 276 Recipes. The other 24 ship and can be named
