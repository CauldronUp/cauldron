...
| Area | State |
|---|---|
| Detection engine (Composer, npm, Go modules) | Working. 274 of 302 Recipes are reachable from a dependency |
...
| Conformance suites (`cauldron verify`) | Working. 3020 cases, 705 of them checked against a live API |
| Spec drift (`cauldron drift`) | Working. 11 Recipes are checked against their provider's own OpenAPI document, 1 provider publishes one this cannot read, and 290 publish none |
...
That second line is the honest one. Of every Recipe: 3020 cases, 705 run against
...
The seven hundred and five are the cases whose provider can be asked without a key. Six are
...
| `undeclared` | The provider publishes nothing to check. 290 of the 302 |
...
The table reaches 274 of the 302 Recipes. The other 28 ship and can be named
