...
| Area | State |
|---|---|
| Detection engine (Composer, npm, Go modules) | Working. 533 of 818 Recipes are reachable from a dependency |
...
| Conformance suites (`cauldron verify`) | Working. 11045 cases, 4042 of them checked against a live API |
...
That second line is the honest one. Of every Recipe: 11045 cases, 4042 run against
a live account and 7003 not. Documentation-derived cases are worth having,
and they are not the same as watching the provider do it. Adding a `verified:`
date to a case is a claim that someone did.

All 4042 are the cases whose provider can be asked without a key. Every other
...
| `undeclared` | No description is recorded for this Recipe. 565 of the 818, and falling -- see `cauldron discover` |
...
provider publishes nothing -- 565 of the 818 here -- is served exactly as it
...
The table reaches 533 of the 818 Recipes. The other 285 ship and can be named
