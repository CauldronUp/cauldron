...
| Area | State |
|---|---|
| Detection engine (Composer, npm, Go modules) | Working. 533 of 825 Recipes are reachable from a dependency |
...
| Conformance suites (`cauldron verify`) | Working. 11121 cases, 4083 of them checked against a live API |
...
That second line is the honest one. Of every Recipe: 11121 cases, 4083 run against
a live account and 7038 not. Documentation-derived cases are worth having,
and they are not the same as watching the provider do it. Adding a `verified:`
date to a case is a claim that someone did.

All 4083 are the cases whose provider can be asked without a key. Every other
...
| `undeclared` | No description is recorded for this Recipe. 572 of the 825, and falling -- see `cauldron discover` |
...
provider publishes nothing -- 572 of the 825 here -- is served exactly as it
...
The table reaches 533 of the 825 Recipes. The other 292 ship and can be named
