...
| Area | State |
|---|---|
| Detection engine (Composer, npm, Go modules) | Working. 525 of 673 Recipes are reachable from a dependency |
...
| Conformance suites (`cauldron verify`) | Working. 9607 cases, 3406 of them checked against a live API |
...
That second line is the honest one. Of every Recipe: 9607 cases, 3406 run against
a live account and 6201 not. Documentation-derived cases are worth having,
and they are not the same as watching the provider do it. Adding a `verified:`
date to a case is a claim that someone did.

All 3406 are the cases whose provider can be asked without a key. Every other
...
| `undeclared` | No description is recorded for this Recipe. 466 of the 673, and falling -- see `cauldron discover` |
...
provider publishes nothing -- 466 of the 673 here -- is served exactly as it
...
The table reaches 525 of the 673 Recipes. The other 148 ship and can be named
