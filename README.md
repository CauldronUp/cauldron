...
| Area | State |
|---|---|
| Detection engine (Composer, npm, Go modules) | Working. 525 of 672 Recipes are reachable from a dependency |
...
| Conformance suites (`cauldron verify`) | Working. 9597 cases, 3398 of them checked against a live API |
...
That second line is the honest one. Of every Recipe: 9597 cases, 3398 run against
a live account and 6199 not. Documentation-derived cases are worth having,
and they are not the same as watching the provider do it. Adding a `verified:`
date to a case is a claim that someone did.

All 3398 are the cases whose provider can be asked without a key. Every other
...
| `undeclared` | No description is recorded for this Recipe. 465 of the 672, and falling -- see `cauldron discover` |
...
provider publishes nothing -- 465 of the 672 here -- is served exactly as it
...
The table reaches 525 of the 672 Recipes. The other 147 ship and can be named
