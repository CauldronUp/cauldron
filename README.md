...
| Area | State |
|---|---|
| Detection engine (Composer, npm, Go modules) | Working. 533 of 801 Recipes are reachable from a dependency |
...
| Conformance suites (`cauldron verify`) | Working. 10881 cases, 3974 of them checked against a live API |
...
That second line is the honest one. Of every Recipe: 10881 cases, 3974 run against
a live account and 6907 not. Documentation-derived cases are worth having,
and they are not the same as watching the provider do it. Adding a `verified:`
date to a case is a claim that someone did.

All 3974 are the cases whose provider can be asked without a key. Every other
...
| `undeclared` | No description is recorded for this Recipe. 560 of the 801, and falling -- see `cauldron discover` |
...
provider publishes nothing -- 560 of the 801 here -- is served exactly as it
...
The table reaches 533 of the 801 Recipes. The other 268 ship and can be named
