...
| Area | State |
|---|---|
| Detection engine (Composer, npm, Go modules) | Working. 533 of 775 Recipes are reachable from a dependency |
...
| Conformance suites (`cauldron verify`) | Working. 10627 cases, 3863 of them checked against a live API |
...
That second line is the honest one. Of every Recipe: 10627 cases, 3863 run against
a live account and 6764 not. Documentation-derived cases are worth having,
and they are not the same as watching the provider do it. Adding a `verified:`
date to a case is a claim that someone did.

All 3863 are the cases whose provider can be asked without a key. Every other
...
| `undeclared` | No description is recorded for this Recipe. 550 of the 775, and falling -- see `cauldron discover` |
...
provider publishes nothing -- 550 of the 775 here -- is served exactly as it
...
The table reaches 533 of the 775 Recipes. The other 242 ship and can be named
