...
| Area | State |
|---|---|
| Detection engine (Composer, npm, Go modules) | Working. 278 of 308 Recipes are reachable from a dependency |
...
| Conformance suites (`cauldron verify`) | Working. 3067 cases, 752 of them checked against a live API |
| Spec drift (`cauldron drift`) | Working. 11 Recipes are checked against their provider's own OpenAPI document, 1 provider publishes one this cannot read, and 296 publish none |
...
That second line is the honest one. Of every Recipe: 3067 cases, 752 run against
...
The seven hundred and fifty-two are the cases whose provider can be asked without a key. Six are
...
| `undeclared` | The provider publishes nothing to check. 296 of the 308 |
...
The table reaches 278 of the 308 Recipes. The other 30 ship and can be named
