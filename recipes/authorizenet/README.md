...
**5 conformance cases, 3 checked against the live API on 2026-09-06.**
...
**The body begins with a UTF-8 byte-order mark.** The first three bytes of that response are `EF BB BF`, before the `{`. Python's `json.loads` on the decoded text raises `Unexpected UTF-8 BOM (decode using utf-8-sig)`; Go's `encoding/json` refuses it too. A client that works against a fake without one throws on its first real response.
...
- **The BOM is not served.** The response writer here encodes JSON and cannot prefix bytes. The evidence is quoted above byte for byte so nobody has to rediscover it; closing it needs a change to the writer rather than to this Recipe.
