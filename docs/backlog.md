...
| Airship | Assess — push channels, named users, segments. Probed 2026-09-09 and **blocked on documentation**: docs.airship.com renders from JavaScript at every URL tried, so the record shapes cannot be read without executing the page. Two findings are already banked for when it can be: the 401 body is **JSON served as `Content-Type: text/plain`** — `{"ok":false,"error":"Unauthorized","error_code":40101}` — which is the fifth content-type mislabel in this collection after Statsig, Wrike, Maxio and Affinity; and `error_code` is a **numeric code whose first three digits are the HTTP status**, 401 followed by 01. Best of all, **the challenge header changes scheme depending on what you sent**: `WWW-Authenticate: Basic realm="API"` when no credential is sent and `Bearer realm="API"` when a Bearer token is, so the header that exists to tell a caller which scheme to use instead echoes the one they already used. The 404 is `text/html` reading "Error 404" |
...
another. These seventy-five are left, and every one of them has now been looked for
