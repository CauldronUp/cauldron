...
| CloudAMQP | RabbitMQ instances and queues. Probed 2026-09-09 and **blocked on documentation**: docs.cloudamqp.com serves a JavaScript shell -- 36 characters of text under a megabyte of bundle -- at every URL tried, so there is no success shape to write against without executing the page. The failure paths are worth the visit when it renders: `GET /api/instances` answers **401 with `WWW-Authenticate: Basic realm="Customer API"`**, correctly double-quoted, beside `{"error":"Not authorized"}`. That is the counterexample to Buffer, whose challenge header is single-quoted and therefore unparseable by a conforming client, and it is worth shipping for that pairing alone. One response to a missing credential, a wrong one and an unknown path alike |
...
another. These seventy-four are left, and every one of them has now been looked for
