...
| Provider | Why |
|---|---|
| ~~Microsoft Graph~~ | Shipped as `msgraph`. **11,493 paths and 17,777 operations in 44MB** -- the largest description here by a wide margin, three times Infisical's bytes and eight times its operations, and titled "OData Service for namespace microsoft.graph" because it is a CSDL translation rather than something written. 88% of the path keys need quoting; the longest path is 340 characters and takes five identifiers to name one file. **A wrong token is answered with IDX14100 and a lesson about JWS and JWE serialisation** -- Microsoft.IdentityModel's own help text on the wire, the most useful refusal in its batch and a library internal that changes when the library does. `request-id` and `client-request-id` carry the same value, so the field named for the caller is the server's. The timestamp in the failure has no timezone, and one small object uses three naming conventions. An unknown path answers 401, so for 11,493 paths the one thing a caller wants to check is the one thing they cannot |
...
| ~~Microsoft OneDrive~~ | Shipped as part of `msgraph` -- the same surface, the same document, the same failures |
| ~~Microsoft Teams~~ | Shipped as part of `msgraph` -- the same surface, the same document, the same failures |
...
| ~~Confluent Cloud~~ | Shipped as `confluent`. **The correlation id is on every failure except the one you would report**: both credential failures carry a fresh 32-hex `id` and the routing failure does not, while gaining a `code` the other two lack -- so no two failures share a key set and the field a support ticket needs is missing from the one most likely to produce it. `source` is `{}` on everything a public probe can reach, so JSON:API's pointer to the part of the request at fault says nothing and still costs a key. There is no `title`, only `detail`, so the short form a list would show has to be derived from the sentence. The wrong-key message is a lesson about key types -- Cloud, Global and Cluster keys look identical and only one opens this surface -- and serving it verbatim is the useful half of the Recipe. Records carry `kind` and `api_version` like Kubernetes objects. The cursor is under `metadata` rather than `links.next` |
...
| ~~CloudAMQP~~ | Shipped, as the instance API. **The challenge header is sent only to callers who already know the scheme**: a request with no credential gets 401 with no `WWW-Authenticate` at all and an empty body, and a request with a wrong one gets `WWW-Authenticate: Basic realm="CloudAMQP API"` and a sentence. RFC 9110 makes that header mandatory on a 401 and it exists to tell an unauthenticated caller which scheme to use; this withholds it from exactly that caller and sends it to the one who has already proved they know. **And CloudAMQP's other API does it correctly** -- customer.cloudamqp.com sends the header to both, so one vendor runs two APIs that disagree about it. The credential is **Basic with an empty username and the key as the password**, the mirror of Close and Insightly, and OpenAPI cannot say that so the document invents `x-username-optional` and `x-default-username` to. The document declares only a 200, the same silence Middesk publishes. And a node carries two of everything: two hostnames, two disk sizes whose total is neither, and two booleans that both read as is-it-up |
...
| Amazon Pinpoint | Messaging and delivery events. Behind SigV4 like the other AWS Recipes here, so the failure paths cost a signing implementation before they say anything |
...
| ~~Bird~~ | Shipped. **The credential scheme is a product that no longer exists**: `Authorization: AccessKey <key>`, which Bird inherited from MessageBird before the rename, on `api.bird.com` -- so no HTTP library has a helper for it and the refusal does not mention it. The third provider here whose name and wire format disagree, after Maxio at chargify.com and Terraform Cloud at app.terraform.io, and the first where it is the **credential** rather than the hostname carrying the old name, which is the harder one to notice. The refusal itself is `{"code":"Unauthorized","message":"The request is not authorized"}` -- the reason phrase in the field a client branches on, and the same fact written out passively beside it, naming no actor and no reason. One response to three mistakes. The cursor is `pageToken` going out and `nextPageToken` coming back, and the region is per workspace rather than per organisation, so one account can span jurisdictions |
...
| ~~WhatsApp Business Platform~~ | Shipped as `metagraph`, which is the surface underneath it. **Every credential failure is HTTP 400**, so the status a client re-authenticates on never arrives and the one it reads as "my request is malformed" is the one meaning "your token expired". The codes are Meta's own small integers -- 2500 for a missing token and 190 for an unparseable one, and 190 is the number every Facebook developer knows by heart. `type` is OAuthException for both, so the field that looks like a category is a constant. An unknown path answers the token error rather than a 404. **A listing of pages hands back an access token per page**, so the response a client logs contains as many credentials as the user has pages. Permissions are a list of verbs rather than a role, so "can this person post" is an array membership test with no name for the set |
...
| ~~Adobe Acrobat Sign~~ | Shipped, with a `spec:` recorded. **Three failures, three codes, and every sentence is true** -- `NO_AUTHORIZATION_HEADER` naming the header rather than the key, `INVALID_ACCESS_TOKEN` saying "invalid **or has expired**", and `NOT_FOUND` -- which after seven providers here answering "invalid key" to a request holding none is the counterexample. **And the published OpenAPI document says the body is something else**: its own 401 examples are `{"reason": "NO_AUTHORIZATION_HEADER: Authorization header not provided.", "status": 401}` -- two different field names, the code and the sentence **concatenated by a colon**, and a full stop the wire does not send. The schema beside those examples is `{"type": "object"}` with no properties, so the examples are the only description of the body and they are wrong about every part of it. The document also declares `Authorization` **`required: false`**, and its `servers` entry is a bare path with no host on an API that shards by region. **`id` and `status` are not in `required` and `hidden` is**, so a conforming agreement must say whether it is hidden from you and need not say what it is called. And the status is written from the reader's point of view: eight of thirty values begin `WAITING_FOR_MY_`, so a cached agreement is only true for the account that fetched it |
| ~~Ironclad~~ | Shipped. **One response to three mistakes, padded with spaces**: `401 { "code": "UNAUTHORIZED", "message": "invalid authentication token" }`, byte-identical for a missing credential, a wrong one and an unknown path -- including the space after the opening brace and before the closing one, which is a serialiser configured for readability rather than bytes and the same class of tell as Gong's space before each colon and Tinybird's after. The sentence describes the wrong failure, the seventh provider here to do it, and is uncapitalised beside a code in screaming snake. **An unknown path is a 401**, so for a contracts API whose whole surface is behind an account nothing about it is discoverable from outside. The path segment says `public`, a word only meaningful inside the company. Every property is an object with a `value` inside it and an unset one is absent rather than a wrapper holding null, so two records in one listing have different property sets |
...
| ~~Sage~~ | Shipped. **Two failures on one route sharing no field, no shape and no naming convention**: a missing credential answers `{ "error": "Authorization missing. ..." }`, an object with one field, and a wrong one answers a **bare top-level array whose every key begins with a dollar sign** -- `[{"$severity":"error","$dataCode":"AccessTokenVerificationError",...}]`. So `body.error` is undefined for one of the two, a client has to index before it can read anything, and `$severity` is not an identifier in any language that maps JSON onto objects. `$source` names the header, which is the useful fact, and it is on the shape that is hardest to read. The listing envelope is dollar-prefixed too (`$items`, `$total`), so the one thing success and failure share is a sigil -- and the request parameters are snake case (`items_per_page`) where the response fields are dollar-prefixed camel case (`$itemsPerPage`), so the name a client sends and the name it reads back are spelled two ways |
...
| ~~Wave~~ | Shipped. **The code is right and the sentence is wrong twice**: `UNAUTHENTICATED`, which is exact, beside "Invalid request, authentication expired." said to a well-formed request that carried no credential at all -- nothing was invalid and nothing expired. Adobe Acrobat Sign, shipped in the same batch, writes the honest version of that sentence about a request that actually held a token. **`data` is `{"businesses": null}` where Railway's is `null`** for the same failure at the same status: the field nulled and the document not, both permitted by the specification, so the difference is not a bug anybody can report and the same client code reads two different things. **Inline arguments are refused outright** -- "Provide data using GraphQL variables" -- so every example copied out of a GraphQL tutorial fails and introspection cannot be written the way the specification writes it; the policy is defensible and no message says why. That refusal is `GRAPHQL_VALIDATION_FAILED` at **HTTP 200** where Railway sends 400. Every error carries a fresh UUID at `extensions.id`. And `businesses` is a **Relay connection that pages by number**: edges and node, with `currentPage`/`totalPages`/`totalCount` and no cursor anywhere |
...
| ~~Imgix~~ | Shipped as `imgix`, and **nothing is mapped to it on purpose**: `@imgix/js-core` names `imgix.net` 75 times and `api.imgix.com` not once, because it builds URLs for the rendering service rather than calling the Management API -- the Stream Chat shape again, and further, because here the sibling product is the one nearly everybody means. **The 401 discloses the running build to the patch**, `3.213.15`, from an anonymous request, along with `healthy`, `read_only` and `tombstone` -- and that second half is genuinely useful and almost unique here: a client can tell a maintenance window from a bug on the response that failed, with no status page and no second request. `meta.authentication` says which scheme was tried in structured fields rather than in the sentence. **A missing credential and a wrong one are byte-identical** apart from a per-request id, so the two failures needing different fixes cannot be told apart. `status` is a string inside the error and a number outside it. The failure carries two unrelated 32-hex correlation ids -- one in the entry, one in `x-cloud-trace-context` -- and nothing says which to quote. Media type is `application/vnd.api+json`. Source attributes are recorded as a gap rather than guessed: the reference page is rendered by JavaScript and has nothing in its HTML |
...
| ~~Aircall~~ | Shipped. **Three statuses for three states, and every one of them is right**: 401 with no credential, 403 with a wrong one, 404 on a path that does not exist -- which is what RFC 9110 says those three mean, and which almost nothing else in this collection manages from one endpoint. The body is the reason phrase copied into `message` and nothing else, so it carries no code, no correlation id and no link: the exact opposite trade to Acuity, which puts the status in the body three times and answers everything 401. **Routing runs before the credential**, so `/v1/ping` with no key is a 401 and `/v1/` with no key is a 404 -- a caller with a typo is told about the typo, and the whole surface is enumerable anonymously. A `Bearer` prefix on a Basic API is a 403, so the one mistake this status set could have named exactly is the one it rounds off. Also pinned: four integer fields in seconds where three are instants and one is a duration; a missed call with **no `answered_at` at all** rather than null; `next_page_link` a whole URL and `previous_page_link` null-and-present on the first page; and `meta.count` and `meta.total` one word apart with the smaller one called count |
| ~~Dialpad~~ | Shipped. **The error envelope is Google's JSON-C format, on an API that is not Google's** -- nested `error`, numeric `code`, an `errors` array carrying `domain`, `message` and `reason`, and the message repeated at both levels. Google deprecated it in favour of google.rpc.Status, and Firebase Auth in this collection already serves the newer shape from the same company. `domain` is "global" and `reason` is "required" -- Google's own enumerations, saying nothing about Dialpad -- and `required` is the answer to supplying a credential, because a missing key and a wrong key are one body. **An unknown path is App Engine's plain-text handler** echoing the path, so both halves of the failure behaviour are Google infrastructure showing through. Identifiers are nineteen-digit Datastore ids sent as strings, with no note anywhere. Contact details are arrays even when there is one, so there is no primary and a client showing "the" number is choosing an index |
...
| ~~8x8~~ | Shipped as `eightbyeight`. **The request id is the same on two different requests** -- struck live seconds apart, with and without a credential, the same 32 hex characters, and a different value only when the path changed. So it is derived from the request rather than minted per call, and a caller reporting a failure hands over a value that also belongs to everybody else who made that call. It is also the only reproducible correlation id here. **Everything is Kong** -- the same sentence to a credentialed request, an uncredentialed one and the host root -- so the gateway is the entire observable API, and this is the third Kong sighting after Productboard and the Fastify leak in Browserbase. A credential failure is reported as a missing route, because the route is mounted per authenticated consumer. The listing is Spring Data's Page on the wire, `number` and all |
...
| ~~Wrike~~ | Shipped. **The same body arrives as `text/plain` on a route Wrike serves and `application/json` on one it does not** -- byte-identical JSON both times, struck live, so the header is right on the path that does not exist and wrong on the one that does. Any client checking the type before parsing refuses the refusal it will really get and parses the one it gets only when the URL is wrong. An unknown path is a 401 rather than a 404, so a path cannot be shown not to exist without a working token. Two credential failures, two accurate sentences, one code. The prose field is `errorDescription`. A task's `status` is one of four fixed values and the board's own column is `customStatusId`, a second request away, so two tasks both reading Completed can be in different places. The permalink carries a decimal id that appears nowhere else on the record. Paging is opt-in: no `pageSize`, no cursor, and a complete answer looks like a first page |
...
| ~~Unit~~ | Shipped -- on the terms the row below set, and not on the card shape. See there |
...
| ~~Affirm~~ | Shipped. **The parameter check runs before the credential check**: with no Authorization header at all, `GET /api/v1/transactions` answers `400 application/problem+json` naming the query parameter it wanted -- so the first thing an unauthenticated caller learns is the name of something it did not send. And the parameter is a **required enum with exactly one legal value**, so it distinguishes nothing except a request that remembered it. Add it and the same credential-less request becomes a 401 in a **completely different error model**: `{status_code, type, code, message}` beside the RFC 9457 `{detail, status, title, type}` above it, sharing not one field name, with `type` a bare word in one and a URI in the other. `about:blank` is the fifth problem document here and the only one that declares itself empty. The two credential failures are told apart with two true sentences, which is rare here. The 404 is a **styled marketing web page**, so one endpoint answers three content types. Also pinned: `ListTransactions` sends `has_next`/`has_prev` and **no cursor at all**, while the sibling events endpoint sends URL-encoded page tokens; the reference calls `provider_id` a string and the schema an integer; the reference's example JSON **does not parse** (`"id: "A1B2-C3D4"`); and the schema documents its own `fee` field as possibly inaccurate for accounting |
...
| ~~Firebase Auth~~ | Shipped, and it earned its place on the failure path the official emulator does not have to model. **The document describes no error at all**: 61 schemas, no envelope, and `EMAIL_NOT_FOUND` and `INVALID_PASSWORD` -- the two strings every client switches on -- appear nowhere in 183KB. The only errors it describes are the per-row ones that arrive inside a 200. Struck live: **a missing credential is 403 and a wrong one is 400**, so the API never sends 401 for either and `if (status === 401) refresh()` fires on neither, and a wrong key is reported in the same class as a malformed body. **The error format is a query parameter called `$.xgafv`** -- enum 1 and 2, percent-encoded as `%24.xgafv` -- and v2 drops the legacy `errors` array while keeping `details`, so one failure has two shapes and neither is written down. The message arrives three times in one body. The reason a program should branch on, `API_KEY_INVALID`, is at `error.details[N].reason` in an array indexed by `@type` rather than by position. **41 methods, 35 of them POST, no DELETE anywhere** -- deleting an account is `POST accounts:delete` and reading them is `POST accounts:lookup` -- so the verb carries no information. **One account record holds five timestamps in four encodings**, and `createdAt` (milliseconds) and `validSince` (seconds) have byte-identical type declarations. 13 response fields say "Always present" in English because Discovery has no `required` list for responses, and `registered` is deprecated *and* documented "Always true". 49 deprecated properties, 13 of them `kind`. **`writeOnly` is used 0 times in 446 properties**, in a document that declares `password`, `passwordHash` and `salt`. And it is a **Google Discovery Document, not OpenAPI**, so nothing can fingerprint it and the Recipe names no `spec:` -- the Codecov call for the opposite reason |
| Coinbase Commerce | Assess — charges that expire, underpayment and overpayment as distinct outcomes. Probed 2026-09-07 and **unreachable**: `api.commerce.coinbase.com` answers 503 with a Cloudflare interactive challenge to any plain request, credential or no credential, so the failure paths cannot be recorded without defeating bot protection. Not retried |
| ~~Circle~~ | Shipped. **A not-found is `code: -1`** -- every other failure's code is its HTTP status, and the router's own is a negative sentinel documented nowhere. **Three different 401s and the shape of the key picks which**, all struck live: nothing gets "malformed authorization...", a wrongly-shaped key gets a four-sentence tutorial that **prints example key material** -- a sample key id and a sample secret -- into a response anyone can trigger without an account, plus a caveat that the format only applies to keys issued after May 2023 and no word on the older one; a rightly-shaped key nobody issued gets "Invalid credentials." Two of the three start lower case and end without a full stop and the third does neither, so one status has three registers. **The health check is outside the API**: `/ping` answers `{"message":"pong"}` with no `code` and no credential, while `/v1/ping` -- the same name one segment in -- is a 404, so a monitor pointed at the versioned surface reports an outage that is a routing table. Money is a string with its places kept, `"0.00"` rather than `"0"`, and there is no scale field to say how many to expect |

## Feature flags and experimentation

| Provider | Why |
|---|---|
| ~~Statsig~~ | Shipped. **Three different mistakes, one response, and it is not JSON**: no credential, a wrong key and an unknown path all answer 401 with the eleven bytes `Unauthorized` and **no Content-Type header at all**, so a client cannot even ask what it was handed and cannot discover whether a route exists. A defensible posture, and worth saying plainly that the design giving an attacker nothing gives a developer nothing. The key prefix names which of three surfaces it opens -- `console-`, `secret-`, `client-` -- and the wrong one is refused like no key at all. A gate's id is the name somebody typed, so renaming one changes its identifier. A disabled gate is still evaluated and still counts checks, so traffic on it is the normal case. The last editor is a display name with no id or email, so the field cannot be joined to a user |
...
| ~~Honeybadger~~ | Shipped. **`errors` is a string.** Plural, holding two words, where every other provider here that names a field `errors` puts an array in it -- so `errors[0]` is `"A"` and `errors.length` is 12 for any client written against the plural. **A missing credential is 403 and so is a wrong one**, byte-identical: 403 means identified and not permitted, and sending nothing is neither, so the status that would tell a client to authenticate never arrives. An unknown path is HTML. A project counts its faults twice -- lifetime and unresolved -- and a dashboard reading the first shows 412 problems where there are 7. Environments are bare strings with no identifier, so one cannot be addressed except by a name a client happens to hold. Reporting and reading take different credentials in different headers |
| ~~Grafana Cloud~~ | Shipped as `grafanacloud`. **"Login Required" is a browser sentence on an API** -- there is no login on this surface, only service-account and access-policy tokens, so the message is accurate about a completely different request reaching the same middleware. The two failures do carry different codes and different sentences, which is more than most providers here manage, and one of the sentences is wrong. "Token could not be parsed" is precise about the right thing: unreadable rather than unrecognised. One correlation id, a UUID, in the body with no header twin -- simpler and better than the two-that-disagree shape Gong and imgix have. A record carries a numeric id and a slug, and every URL uses the slug |
...
| LogRocket | Assess — sessions and issues. Probed 2026-09-07 and **blocked on documentation**: docs.logrocket.com serves a Cloudflare interactive challenge to a plain fetch, and its public reference is SDK guides with no REST API pages at all, so there is no success shape to write against. The live failure paths are worth the visit when the documentation is reachable: `/v1/orgs/{org}/apps` answers **301 to the same path with a trailing slash** — Django's APPEND_SLASH, before authentication, so an unauthenticated caller learns the canonical URL — and the slashed path answers **403 `{"detail":"Authentication credentials were not provided."}`, which is Django REST Framework's canonical sentence and DRF's documented 403-instead-of-401 when no authenticator supplies a challenge. The sentence is exactly right and the status is exactly wrong. It also sends `Allow: GET, POST, HEAD, OPTIONS` on the refusal, so a rejected request still enumerates the methods. And **a wrong credential is worse than none**: `Authorization: Token notreal` answers **500 with Django's bare HTML error page**, so sending a credential turns a clean typed refusal into an unhandled exception |
...
| ~~Railway~~ | Shipped, and the GraphQL-only exclusion this row already called lapsed is now lapsed twice over. **A refusal is HTTP 200 and a typo is HTTP 400**: an unauthenticated query comes back 200 with `"code":"INTERNAL_SERVER_ERROR"` beside the message "Not Authorized", and `{ nope }` comes back 400 with `GRAPHQL_VALIDATION_FAILED`. So the machine-readable half of a **permanent** failure says the server broke -- Apollo's default for a resolver that threw without a code -- and every retry policy ever written treats that as transient, at 200, forever. The accurate code is on the mistake a developer makes once and the wrong one on the mistake every integration makes first. `data` is **null on the 200 and absent from the 400**. `traceId` is an eighteen-digit number as a string on the GraphQL path and `requestId` is a UUID on the REST 404, so one service has two identifier names in two formats and neither response mentions the other. And **introspection is open to anonymous callers**: `apiTokens`, `auditLogs`, `bucketS3Credentials`, `botScopeBindings` and `adminVolumeInstancesForVolume` are all readable as names by a request that cannot read one field |
...
| ~~Infisical~~ | Shipped. **The document contains zero `$ref`s** -- not few, none, in 12,974,420 bytes with an empty `components.schemas`. 16,097 response schemas are declared and 869 are distinct, so 94.6% of the file is a copy of something else in it; the 400 envelope alone is written out 2,298 times and the six error statuses account for 13,788 inline copies of five fields. Every one of those declares `required: [reqId, statusCode, message, error]` with `additionalProperties: false` -- **and the real 404 has no `reqId`**, struck twice on two unknown paths, because it is Fastify's own handler answering from below whatever stamps the field on everything else. **A credential that cannot be parsed is a 403**: missing is 401 UnauthorizedError, unreadable is 403 TokenError, and a real-shaped JWT with a bad signature is 403 again with `invalid signature` -- jsonwebtoken's own string, lower case and without a full stop, beside a sibling that is a written sentence. So the refresh-on-401 branch never fires on a stale token. `reqId` names the region that answered. **Not one of the 2317 operations has a summary** and 300 have no operationId. The role listing has no paging of any kind and `additionalProperties: false` leaves nowhere to add one. `description` is nullable and not required, so absent and null are both legal for the same fact. Version 0.0.1, on two production regions |
| ~~Terraform Cloud~~ | Shipped as `terraformcloud`. **Every failure is two fields and neither is a sentence**: `status` restates the HTTP status as a string and `title` is the status phrase, lower-cased, with no detail, code, id or source anywhere -- so the body of a failure is the status line twice and a client reading it learns nothing. JSON:API reserves `title` for the summary that does not change between occurrences, and only that half is here. A missing credential and a wrong one are byte-identical, and no field could have separated them. The 404 has the same shape, which is the good half: one parser reads everything, and HCP Terraform buys that uniformity by never saying anything. Field names are hyphenated, paging is under `meta.pagination` rather than `links.next`, and the identifier is the organization name somebody typed. Renamed to HCP Terraform in 2024 and still served from app.terraform.io |
| ~~Pulumi Cloud~~ | Shipped as `pulumi`. **The refusal says "or"**: "Unauthorized: No credentials provided or are invalid." -- one sentence covering both cases, byte-identical whether a credential was sent or not. Honest, where most providers here pick a side and pick the wrong one, and still not actionable, because a client cannot branch on a conjunction. The status appears three times in one exchange. **An unknown path is `text/plain` and it is Go's** -- http.NotFound left unreplaced -- so the JSON envelope belongs to routes that exist and the failure a client meets while getting a URL wrong is the one .json() throws on. The scheme is `token` rather than `Bearer`, so every library's bearer helper is refused by a sentence that never mentions the scheme. A stack has no id: organization, project and stack, none unique alone. A stack that never ran omits lastUpdate rather than sending a zero that would sort to 1970, and the time is seconds where most of this collection sends milliseconds |
...
| ~~Trigger.dev~~ | Shipped. **One endpoint, two error models, and which you get depends on which mistake you made**: no credential answers `application/problem+json` with a full RFC 9457 document, a wrong one answers `application/json` with `{"error":"Invalid API key"}`. So a client cannot write one parser for a 401 from this route. **The problem `type` points at MDN's page about HTTP 401** -- a correct page about a status the document already carries twice, and nothing about Trigger.dev, in the field RFC 9457 reserves for identifying this provider's problems. `detail` and `error` carry the same string, so the document is five fields holding four facts. An unknown path is HTML from the dashboard app. Paging uses bracketed `page[size]` and `page[after]`, and the cursor is nested under a pagination object. Test runs share the listing with production ones, separated by a boolean nobody has to filter on |
...
| ~~Acuity Scheduling~~ | Shipped. **Three fields, one fact, three renderings**: `{"status_code":401,"message":"Unauthorized","error":"unauthorized"}` -- the number, the reason phrase capitalised, and the reason phrase lower-cased, every one of them derivable from the status line. The most compact example here of a pattern beehiiv and Terraform Cloud also have, and the only one where the body carries nothing at all the status did not. The two phrase fields disagree about case in one object and neither name says which is the one to branch on. Also pinned: **the start is a full offset timestamp and the end is `"10:45"`**, a wall clock with no date and no zone, so a duration needs two fields and an appointment crossing midnight cannot be represented; the offset is `-0400` without a colon, which a strict RFC 3339 parser refuses; a cancelled appointment is in the listing with no default filter; and the page size is called `max` |
...
| ~~Stream~~ | Shipped as `streamchat`, named for the product rather than the company because the npm package literally called `getstream` is the client for Activity Feeds -- a different Stream product on a different host, which is why detection excludes it. **The 401's message names the internal handler**, so the same failure reads "GetApp failed with error...", "QueryMembers failed with error..." and "GetReactions failed with error..." on three routes struck live; the prefixes are the document's own operationIds and a client matching on the sentence needs all 76. Absent and wrong credentials share a status and a code and differ only inside the escaped quotes. **`StatusCode` is PascalCase** among five lower-case siblings, and there are two numbers on every failure -- `StatusCode` 401 beside `code` 2, or 404 beside 16. **A request wants three credentials in three places**: the document ANDs a JWT header, a query api_key and a Stream-Auth-Type header, and declares the JWT as `type: apiKey` so it goes in Authorization with no `Bearer ` prefix. **Six GETs take a query parameter named `payload` carrying a JSON document** -- filter, sort, limit and offset all inside it, declared with the `content:` form most generators degrade to a string. `sort` is an array with `maxItems: 1`. The real filter grammar lives in `x-stream-filter-fields`, machine-readable in a key no generator reads, beside a `filter_conditions` declared as any object at all. `duration` is required on 85 response schemas, none of which declares a format, and is the string "0" on every failure. A wrong method is a 404. One `deprecated: true` in 348 schemas and it is not the field a caller meets -- the exact inverse of Firebase Auth. 20 enumerations are prose. A channel member has no id, and five separate fields describe its bans. Version v237.27.1, on unversioned paths |
...
| ~~Iterable~~ | Shipped. **The 401 hands back the caller's IP address and one of Iterable's own**: `ip` is the forwarded-for chain joined with a comma, and the second element was 10.10.50.35 on one request and 10.10.49.6 on the next from the same client minutes apart -- so an unauthenticated refusal names the internal node that answered, and enough of them map the range. The first element is a header the caller controls, echoed into a body readable without a credential. `params.endpoint` echoes the path. An unknown path is HTML, so the JSON 401 is only reachable where routes exist. The prose field is `msg` and the field named `code` holds the word "Unauthorized" on both failures, so the machine-readable half carries no information. Creation times are milliseconds as a bare Number. A Dynamic list and a Standard one sit in the same array and mean different things by membership |
...
| ~~Attentive~~ | Shipped, with a `spec:` recorded. **A 401 with nothing in it at all**: no body, no `Content-Type`, and no `WWW-Authenticate` -- which RFC 9110 section 15.5.2 makes **mandatory** on that status. It is the only required field on a 401 and the only one absent, so nothing anywhere in the response separates a missing credential from a wrong one or names the scheme. The single header sent is `nosniff`, an instruction not to guess the media type of a body that is not there. Buffer, shipped in the same batch, sends the empty body and puts the whole failure in `WWW-Authenticate`; this one sends the empty half and not the other. **The 404 is the static-file handler** -- Spring Boot's error attributes with `"No static resource v2/cauldron-nope."`, a REST API reporting an unknown endpoint as a missing file -- and the sentence drops the leading slash the `path` field keeps. `requestId` is eight hex, a hyphen, seven hex: the first two groups of a UUID and not a UUID. The document declares 400, 401, 403, 429 and 500 and describes the body of none of them. **A segment has no `id`**: the only identifier is the `externalId` the caller supplied, and one created without it cannot be addressed. And authentication is declared twice, in `security` and in a vendor `x-requires-auth`, which disagree about two of the seven paths |
...
| ~~Telegram Bot API~~ | Shipped, and it is why `auth.scheme: path` now exists: **the credential is the first path segment**, prefixed with the literal `bot`, so it cannot be set as a default header and lands in every log that records a URL -- beside `Access-Control-Allow-Origin: *` on every response, so a browser can call it with the token in the address bar. Alchemy, PubNub, TheSportsDB and Inngest each wrote a paragraph about having no scheme for this and worked around it; Telegram made five. **The shape of the token decides whether the URL exists**, struck live: `abc:def` and a bare number answer 404 Not Found, a well-formed token nobody issued answers 401, so the same wrong secret is a missing endpoint or a refusal depending on how it is misspelled. An unknown method with a bad token is 401, so method existence is not discoverable and the documented case-insensitivity could not be confirmed from outside. **DELETE, PUT and PATCH answer 501 with an empty body and no Content-Type** -- not 405 -- the only three responses this API sends that are not JSON, in an API whose contract is "always has a Boolean field ok". **Telegram disclaims its own error_code** in as many words, leaving prose as the only thing to switch on. **94 fields are typed `True`** rather than Boolean -- present when they hold, absent when they do not -- beside 362 typed Boolean, and `is_premium` and `is_bot` sit on the same object. 100 are typed `Integer or String`. **Reading the second page destroys the first**: `offset` confirms and forgets everything below it, and a negative one forgets the queue, so a GET mutates destructively. Ids are documented as at most 52 significant bits, sized to what a double holds exactly. Chat ids are negative and user ids positive, so the sign of an integer says what kind of thing it names |
| ~~Buffer~~ | Shipped. **The two halves of one failure travel on two channels**: a missing token answers 401 with an **empty body** and the whole failure in `WWW-Authenticate`, and a wrong token answers 401 with a JSON body and **no `WWW-Authenticate` at all**. So a client reading only the body sees a blank 401 for the commonest mistake there is. The header is **single-quoted** -- `realm='Service'` -- which RFC 9110 does not permit for an auth-param value, so a conforming parser rejects the whole challenge; the code is `invalid_request`, which RFC 6750 defines for a malformed request and tells servers to omit entirely when no credential was sent; and the description is **RFC 6750's own definition pasted whole**, five ways to be malformed, none of which happened. `Content-Type: text/html` on zero bytes. **The retirement notice rides on the failure**: the wrong-credential body carries a `deprecation` object with a 2027-02-01 `sunset` date, so the announcement that the whole API is going away reaches only callers who got their credential wrong, in a body rather than in the `Sunset` header RFC 8594 defines. And **the documentation is gone** -- developers.buffer.com is now the GraphQL API only, and the single page describing this surface is the migration guide the failure links to |
...
| ~~Airship~~ | Shipped. **The challenge header advertises whichever scheme you already tried**: no credential gets `WWW-Authenticate: Basic realm="API"` and a Bearer token gets `Bearer realm="API"`, on an endpoint whose own reference lists three accepted schemes and whose neighbour lists four. RFC 9110 lets a 401 carry several challenges so a server can say which it takes; this sends exactly one and picks it by echoing the caller, so the only way to learn Bearer is available is to have already guessed it. The opposite fault to CloudAMQP, shipped the same day, which sends no challenge at all to the caller who needs one. **The 401 is JSON labelled `text/plain` and the reference says so** -- the mislabel is in the documented response table, the fifth content-type fault here after Statsig, Wrike, Maxio and Affinity. **`error_code` is 40101**, the HTTP status with two digits appended, so the code a client branches on contains the status it already had. The reference declares three content types across three responses and the two JSON ones differ -- `application/vnd.urbanairship+json; version=3` on success and the same type without the version on 404. And **the vendor media type names a company that was renamed**: urbanairship is Urban Airship, the fourth rename survivor here after Bird, Maxio and Terraform Cloud, and the first baked into a media type, which is the hardest to change because it breaks content negotiation. Also pinned: a `Data-Attribute` header naming which body key holds the payload, which nothing else here does; the next page in both a Link header and the body, with the response-header table calling Link the *current* page while its own example says rel=next; and two examples on one page naming platform and address `type`/`address` in one and `device_type`/`push_address` in the other |
| Trustpilot | Assess — business units, reviews, invitations. Probed 2026-09-09 and **unreachable**: api.trustpilot.com answers 403 from CloudFront with "The request could not be satisfied. Request blocked.", a WAF rule rather than an API response, so the failure paths cannot be recorded without defeating bot protection. Not retried |
| Height | Assess — tasks and lists. Probed 2026-09-07 and again 2026-09-09 and **unreachable both times**: api.height.app fails the TLS handshake outright (curl exit 35), before any HTTP request is sent, so there is nothing to record about the API at all |
| ActiveCampaign | Assess — contacts, deals, automations. Probed 2026-09-09: the API is on a per-account subdomain, `{account}.api-us1.com`, and an account that does not exist answers **403 with an empty body** — no content type, no JSON, nothing. So the shape Scalekit refuses with four words of plain text, this one refuses with zero bytes. Worth writing when an account name is available to check the success paths against |
| Copper | Assess — people, companies, opportunities. Probed 2026-09-09: `{"error":"authentication error"}`, lower case, no code, no field named. Thin on its own; a candidate to pair with another CRM |
| ~~Metriport~~ | Shipped, and the second Recipe the `health` capability was added for. **Two words, three situations, one status**: a missing credential, a wrong credential and an unknown path all answer 403 `{"message":"Forbidden"}`, byte-identical -- AWS API Gateway's default, so Metriport's own application is never reached and the surface is opaque from outside. It sits in a range this collection now has three points on: Statsig answers eleven bytes and no content type, Metriport two words with one, and Health Gorilla a full OperationOutcome that enumerates 42 resource types -- the last two both clinical APIs at opposite ends of the same decision. Identifiers are UUIDv7, so they sort chronologically and leak when a patient record was created. A patient can be held by several facilities at once |
| ~~Health Gorilla~~ | Shipped as `healthgorilla`, and it is one of the two Recipes the `health` capability was added for. **An unauthenticated 404 lists the entire API surface** -- 42 resource types in a 656-character diagnostics string, HAPI FHIR's default handler naming everything the server can serve, to a request with no credential. The route is resolved before the credential is judged, which is what makes it reachable. **The statuses are inverted**: 403 for sending nothing and 401 for sending something wrong, and the richer body is on the anonymous request, so the failure with something to say is the one nobody had to authenticate for and the failure a real integration hits is empty. The failure is itself a FHIR resource with a discriminator, which is the best-designed error model here and belongs to the specification rather than the provider. `severity` is fatal on the credential failure and error on the routing one. A birth date can be a year and a month, so it is not a date |
...
| ~~Unit~~ | Assessed and left once, then shipped when it earned it. The first assessment was right: an authorization is a hold and the clearing is a second object pointing back at it, and that is the Marqeta Recipe's headline, already shipped. So this one is the banking-as-a-service half instead, and it stands on the deposit account. **The balance is not the money you can spend** -- `balance`, `available` and `hold` are three separate integers in cents and only `available` is spendable, so showing `balance` overstates it by exactly the amount about to leave. The envelope is JSON:API, so the number is at `data.attributes.balance` and `data.balance` is undefined, and `included` beside it holds side-loaded *customers* rather than more accounts. **The content type is `application/vnd.api+json`**, so a framework decoding only `application/json` hands the handler an empty body and reports nothing. Paging is bracketed, `page[limit]` and `page[offset]`. A failure object carries both `detail` and `details`, one letter apart, with `status` as a string inside an array. Frozen is neither open nor closed. The application lifecycle and the counterparty model are still unmodelled and still worth doing |
...
| ~~Maxio~~ | Shipped -- see the row above |
...
| ~~Grafana Cloud~~ | Shipped -- see the row above |
| ~~Better Stack~~ | Shipped. Its 404 hardcodes the word GET whatever method was sent, and it is the only one of its group with both a last-checked time and paused as its own status word |
...
| ~~Statsig~~ | Shipped -- see the row above |
...
| ~~Nango~~ | Shipped. **An unknown API path answers 200 with an HTML page, and the page carries an engineering note about token leakage**: "Auth urls carry tokens in the path. Cloud serves this from a CDN, so the server's Referrer-Policy header doesn't apply. Must stay above any request-initiating tag." The mitigation is correct and carefully reasoned, and the comment is a precise description of where the soft spot is, served with a 200 to anybody who mistypes a path. **The refusal says the credential should be a UUID v4**, specifically -- the most precise credential-shape message in this collection -- and it is accurate about the missing case, where Loops, Helicone and beehiiv all answer "invalid key" to a request carrying none. `payload` is an empty object on every failure. The path is singular and the envelope is plural |
...
| ~~Mercado Pago~~ | Shipped. **A timestamp and a request id joined by a semicolon in one field**: `cause[0].data` is `07-09-2026T21:50:21UTC;b05e2e13-...` -- day-first, `T` in the middle, and the literal letters `UTC` where an offset belongs, so no date library parses it, and the request id a caller would quote to support is concatenated onto the end of it. **"Unauthorized use of live credentials" is the answer to sending none**, the sixth provider here to describe the wrong failure and the most specific about the wrong thing. The 404 is **in Spanish beside an English `error` field** and names MercadoLibre while linking mercadopago.com, so the sentence names one company and the URL another. Also pinned: the status appears three times and the message twice, and the reason a payment is held lives in `status_detail` rather than `status`, which is the row's original note |
| ~~dLocal~~ | Shipped as `dlocal`. **The refusal is a ladder and the first rung is a clock**: no headers answers 400 about `X-Date`, adding X-Date answers 400 about login and key, and a complete but wrong credential answers 403. The timestamp is checked before the credential because it is part of the signature scheme -- defensible, and it means the first failure a new integrator meets names a header they have never heard of. **The same code 5001 covers two different missing things**, and the structured `param` field naming which is present on only one of them. Across the ladder the API answers 400, 400 and 403 to one problem. The leading digit of the code is a category nothing declares. PENDING is the normal state rather than a transient one, because local methods settle over days. `status_code` is a string that reads like an HTTP status, and timestamps use `+0000` where the request's own X-Date uses Z |
...
| ~~Rev.ai~~ | Shipped as `revai`. **A problem document with no problem in it**: RFC 9457 defines five members and two arrive, a sentence and a restatement of the status line. No `type` means about:blank -- "no additional semantics beyond that of the HTTP status code" -- which is exactly true and is the most standardised way in existence of saying nothing. This collection now has three problem+json documents failing three different ways: Trigger.dev points `type` at MDN's page about the status, Agicap at the RFC clause defining 500, and Rev.ai omits it. Three providers, one specification, and not one uses the field to name a problem the API can have. The sentence is ASP.NET's default, passive and naming no actor. A failed job puts its reason in a second field, so `status` is one word for every failure |
...
| ~~Helicone~~ | Shipped. **`trace` is the name of the function that rejected you** -- the constant string `isAuthenticated.error` on every failure, a function and the branch inside it rather than a request id, so the field named for correlation correlates nothing and what it discloses is a name from the codebase. Helicone is open source, so grepping for it finds the check; that is arguably an affordance rather than a leak, and it is still not a trace. **"No API key found" is the answer to sending an API key**, so the two failures are separated only by prose and the prose describes the wrong one, with the field that could have carried the difference occupied by a constant. An unknown path answers 401, so route existence is not discoverable. A read is a POST because the filter is a document, and the paging controls are in the body too -- so a page cannot be bookmarked and a proxy sees one URL for every page. Field names mix snake and camel on one record; cost is a float named for a currency rather than typed for one; and a failed upstream call still cost money, so a dashboard filtered to successes hides the spend it exists to explain |
...
another. These one hundred and eighty-three are left, and every one of them has now been looked for
...
| Mercado Pago | The packages naming Mercado Pago are the same set the Mercado Libre row records from the other side -- unmaintained wrappers with nothing depending on them, spread across several countries' products, and named for the company rather than for this API. Checked 2026-09-07 |
| Sage | Sage publishes SDKs per product and per country, and none of them resolves under a name that identifies the Accounting API this Recipe describes -- so a name match would be a guess about which Sage product a project uses. Checked 2026-09-07 |
| Scalekit | The SDKs are named for the company, and a project holding one is using SSO, directory sync or passwordless -- three different surfaces on a per-customer subdomain this Recipe cannot know. Checked 2026-09-07 |
| Dust | The SDK is named for the company and covers the agent runtime as well as this API, and the workspace is a path segment no dependency name carries. Checked 2026-09-07 |
| Ironclad | No client for this API on npm, Packagist or the Go module proxy under any obvious name, checked 2026-09-07 |
| Acuity | A Squarespace product now, and it publishes no client of its own on any registry, checked 2026-09-07 |
| Bird | The published clients are MessageBird's, for the old API, and none of them calls api.bird.com -- the same split-by-rename miss the Maxio row records from the other side. Checked 2026-09-07 |
| Aircall | No client on npm, Packagist or the Go module proxy under any obvious name, checked 2026-09-07 |
| Affirm | The packages naming Affirm are merchant-platform plugins -- WooCommerce, Magento and Shopify integrations -- rather than clients of the Transactions API this Recipe describes, so a name match would offer a payments emulator to a storefront that never calls it. Checked 2026-09-07 |
| Buffer | The packages that call this API are wrappers of an interface Buffer's own failure body says retires on 2027-02-01, and the surface they target is the one being replaced. Mapping them would point the detector at an API with a sunset date. Checked 2026-09-07 |
| Railway | Driven by its CLI and by generic GraphQL clients pointed at a URL, neither of which a dependency name distinguishes. Checked 2026-09-07 |
...
| convertapi | ConvertAPI is reached through convertapi on npm, PyPI, NuGet or RubyGems, none of which resolves to this host through a dependency file. Checked 2026-09-14 |
| docraptor | DocRaptor is reached through docraptor on RubyGems, npm or PyPI, none of which resolves to this host through a dependency file. Checked 2026-09-14 |
| uploadthing | UploadThing is reached through uploadthing on npm, which does not resolve to this host through a dependency file. Checked 2026-09-14 |
| trigger | Trigger.dev is reached through @trigger.dev/sdk or a plain bearer call, neither of which resolves to this host through a dependency file. Checked 2026-09-14 |
...
| schematic | Schematic is reached through schematic-go, @schematichq/schematic-js or a plain header call, none of which resolves to this host through a dependency file. Checked 2026-09-14 |
...
| applitools | Applitools is reached through @applitools/eyes-* on npm or the SDKs for other languages, none of which resolves to this host through a dependency file. Checked 2026-09-13 |
| clicksend | ClickSend is reached through clicksend/clicksend-php, clicksend on npm, or a plain Basic call, none of which resolves to this host through a dependency file. Checked 2026-09-13 |
| builderio | Builder.io is reached through @builder.io/sdk or a plain URL carrying an apiKey parameter, neither of which resolves to this host through a dependency file. Checked 2026-09-13 |
| momento | The Momento HTTP API is reached through a plain HTTP call carrying a bare Authorization value or a token query parameter, neither of which resolves to this host through a dependency file. Checked 2026-09-13 |
| recall | Recall.ai is reached through a plain HTTP call carrying Authorization: Token, which does not resolve to this host through a dependency file. Checked 2026-09-13 |
| tldv | tl;dv is reached through a plain HTTP call carrying an x-api-key header, which does not resolve to this host through a dependency file. Checked 2026-09-13 |
| dato | DatoCMS is reached through @datocms/cma-client on npm or a plain bearer call, neither of which resolves to this host through a dependency file. Checked 2026-09-13 |
| zerotier | ZeroTier Central is reached through go-ztcentral, the zerotier-central-api crate, or a plain Authorization: token call, none of which resolves to this host through a dependency file. Checked 2026-09-13 |
| hcloud | Hetzner Cloud is reached through hcloud-go, hcloud-python or a plain bearer call, none of which resolves to this host through a dependency file. Checked 2026-09-13 |
| replit | Replit is reached through a plain HTTP call carrying a bearer token beginning rpl_, which does not resolve to this host through a dependency file. Checked 2026-09-13 |
| daytona | Daytona is reached through @daytonaio/sdk on npm or daytona on PyPI, and neither resolves to this host through a dependency file. Checked 2026-09-13 |
| unifiedto | Unified.to is reached through @unified-api/typescript-sdk or a plain HTTP call carrying an authorization header, and neither resolves to this host through a dependency file. Checked 2026-09-13 |
| kombo | Kombo is reached through a plain HTTP call carrying a bearer token and an X-Integration-Id header, which does not resolve to this host through a dependency file. Checked 2026-09-13 |
| fal | fal is reached through @fal-ai/client on npm or fal-client on PyPI, and neither resolves to this host through a dependency file. Checked 2026-09-13 |
| pylon | Pylon is reached through a plain HTTP call carrying a bearer token, which does not resolve to this host through a dependency file. Checked 2026-09-13 |
| depot | Depot is reached through the depot CLI or a plain HTTP call carrying a bearer token, and neither resolves to this host through a dependency file. Checked 2026-09-13 |
...
| vastai | Vast.ai is reached through vastai on PyPI or a plain HTTP call carrying a bearer token, and neither resolves to this host through a dependency file. Checked 2026-09-13 |
| checklyhq | Checkly is reached through the checkly package on npm or a plain HTTP call carrying a bearer token, and neither resolves to this host through a dependency file. Checked 2026-09-13 |
| unstructured | Unstructured is reached through unstructured-client on PyPI or a plain HTTP call carrying an unstructured-api-key header, and neither resolves to this host through a dependency file. Checked 2026-09-13 |
| astronomer | Astronomer is reached through the astro CLI or a plain HTTP call carrying a bearer token, and neither resolves to this host through a dependency file. Checked 2026-09-13 |
| tryfinch | Finch is reached through @tryfinch/finch-api on npm or finch-api on PyPI, and neither resolves to this host through a dependency file. Checked 2026-09-13 |
| twenty | Twenty is reached through a plain HTTP call carrying a bearer token, which does not resolve to this host through a dependency file. Checked 2026-09-13 |
| llamacloud | LlamaCloud is reached through llama-cloud or llama-index on PyPI, or a plain HTTP call carrying a bearer token, and none of those resolve to this host through a dependency file. Checked 2026-09-13 |
| squadcast | Squadcast is reached through its Terraform provider or a plain HTTP call carrying a bearer token, and neither resolves to this host through a dependency file. Checked 2026-09-13 |
| ionos | IONOS Cloud is reached through ionos-cloud/sdk-go, ionoscloud on PyPI, or a plain HTTP call carrying a bearer token, and none of those resolve to this host through a dependency file. Checked 2026-09-13 |
| tailscale | Tailscale is reached through tailscale-client-go, the tailscale CLI, or a plain HTTP call carrying a bearer token, and none of those resolve to this host through a dependency file. Checked 2026-09-13 |
| gandi | Gandi is reached through gandi.cli on PyPI, go-gandi, or a plain HTTP call carrying a bearer token, and none of those resolve to this host through a dependency file. Checked 2026-09-13 |
| ngrok | ngrok is reached through ngrok-api-go, ngrok-api on PyPI, or a plain HTTP call carrying a bearer token, and none of those resolve to this host through a dependency file. Checked 2026-09-13 |
| paperspace | Paperspace is reached through the gradient CLI or a plain HTTP call carrying a bearer token, and neither resolves to this host through a dependency file. Checked 2026-09-13 |
| messagebird | MessageBird is reached through the messagebird package on npm, PyPI or Packagist, and none of those names resolve to this host through a dependency file. Checked 2026-09-13 |
| gumlet | Gumlet is reached through a plain HTTP call carrying a bearer token or through its image URL rewriting, and neither resolves to this host through a dependency file. Checked 2026-09-13 |
| astra | Astra is reached through astrapy on PyPI, astra-db-ts on npm, or a plain HTTP call carrying an AstraCS: bearer, and none of those resolve to this host through a dependency file. Checked 2026-09-13 |
| cockroach | CockroachDB Cloud is reached through cockroach-cloud-sdk-go, the ccloud CLI, or a plain HTTP call carrying a bearer token, and none of those resolve to this host through a dependency file. Checked 2026-09-13 |
| stability | Stability is reached through stability-sdk on PyPI or a plain HTTP call carrying an Authorization header, and neither resolves to this host through a dependency file. Checked 2026-09-13 |
| tinify | Tinify is reached through the tinify package on npm, PyPI, RubyGems or Packagist, and none of those names resolve to this host through a dependency file. Checked 2026-09-13 |
| bytescale | Bytescale is reached through @bytescale/sdk or a plain HTTP call carrying an Authorization header, and neither resolves to this host through a dependency file. Checked 2026-09-13 |
| runpod | Runpod is reached through the runpod package on PyPI or a plain HTTP call carrying a bearer token, and neither resolves to this host through a dependency file. Checked 2026-09-13 |
| creem | Creem is reached through @creem_io/nodejs or a plain HTTP call carrying an x-api-key header, and neither resolves to this host through a dependency file. Checked 2026-09-13 |
| baseten | Baseten is reached through truss on PyPI or a plain HTTP call carrying an Authorization header, and neither resolves to this host through a dependency file. Checked 2026-09-13 |
| gladia | Gladia is reached through @gladiaio/sdk or a plain HTTP call carrying an x-gladia-key header, and neither resolves to this host through a dependency file. Checked 2026-09-13 |
| shortpixel | ShortPixel is reached through shortpixel/shortpixel-php on Packagist or its WordPress plugin, and neither name resolves to this host through a dependency file. Checked 2026-09-13 |
| openmeter | OpenMeter is reached through @openmeter/sdk, the openmeter package on PyPI, or the Go client in its own repository, and none of those names resolve to this host through a dependency file. Checked 2026-09-13 |
| permit | Permit.io is reached through its PDP sidecar and its language SDKs, which are pointed at a locally-running decision point rather than at this management API, so a dependency names the sidecar's client and never this host. Checked 2026-09-13 |
| Jina | Jina is reached through an OpenAI client with the base URL changed, or through curl against a documented endpoint, neither of which names a Jina package. Checked 2026-09-13 |
...
| CloudConvert | The cloudconvert packages on npm and Packagist are real clients, but CloudConvert is most often reached through its Zapier and Make integrations or a signed upload URL, neither of which names a dependency. Checked 2026-09-13 |
| Basin | Basin is reached by pointing an HTML form's action at it, which is markup rather than a dependency, and it publishes no client library. Checked 2026-09-13 |
| PocketBase | The pocketbase package on npm is a real client, but every instance is self-hosted and its address lives in configuration rather than in the dependency. Checked 2026-09-13 |
| UpCloud | UpCloud is driven by its own CLI and by a Terraform provider; upcloud-go-api is a Go module a project holds only when it is building tooling, so a dependency list mostly shows neither. Checked 2026-09-13 |
| deepinfra | DeepInfra is reached through an OpenAI client with the base URL changed, which names no DeepInfra package at all. Checked 2026-09-13 |
| cosmicjs | The @cosmicjs/sdk package is a real client, but a project holding one is pointed at whichever bucket slug and read key its configuration names, and neither is in the dependency. Checked 2026-09-13 |
| Appwrite | The appwrite and node-appwrite packages are real clients, but a project holding one is pointed at whichever endpoint and project id its configuration names -- self-hosted or cloud -- and neither is in the dependency. Checked 2026-09-13 |
| Prismic | The @prismicio/client packages are real clients, but every one of them is pointed at a repository subdomain that lives in configuration rather than in the dependency, and the subdomain is the only thing an emulator would need. Checked 2026-09-13 |
| ButterCMS | The buttercms packages on npm and Packagist are real clients of this API, but a project holding one is pointed at whichever account its read token belongs to, and the same package also talks to the Write API on a different host. Checked 2026-09-13 |
| SignWell | SignWell publishes no first-party client library, and a project using it holds an embed script or a webhook handler rather than a client of this API. Checked 2026-09-13 |
| Clever Cloud | Clever Cloud is driven by its own clever-tools CLI, which a project holds as a dev dependency at most and usually installs globally, so a dependency list rarely names it. Checked 2026-09-13 |
| Civo | Civo is driven by its own CLI and by a Terraform provider, and civogo is a Go module a project would hold only if it were building tooling on top, so a dependency list mostly shows the CLI, which is not a dependency. Checked 2026-09-13 |
| Upsun | Upsun and Platform.sh are driven by their own CLI and by a Terraform provider; the published client libraries are generated and rarely held directly, and either brand's CLI can be pointed at either host. Checked 2026-09-13 |
| Novita | Novita is reached through an OpenAI client with the base URL changed, which names no Novita package at all. Checked 2026-09-13 |
...
| Fathom | A project using Fathom holds a script tag or a proxy route rather than a client of this API, and the packages named for it are tracker wrappers that post pageviews rather than read the reporting surface. Checked 2026-09-13 |
| Aptible | Aptible is driven by its own CLI and by Terraform; the Go and Ruby clients are generated for internal use, so a project holding one is rare enough that a name match would mostly be wrong. Checked 2026-09-13 |
...
| Kestra | Kestra ships SDKs for Java, Python, Go and JavaScript, and every one of them is pointed at whichever self-hosted instance a deployment runs -- so a dependency says the project talks to Kestra and never which host an emulator would have to stand in for. Checked 2026-09-11 |
| Storyblok | The clients on npm and Packagist -- storyblok-js-client, the framework SDKs, the PHP client -- are pointed at whichever of nine regional base URLs a space lives on, and the token does not say which. So a dependency name says the project uses Storyblok and not which host an emulator would have to stand in for. Checked 2026-09-11 |
| Hatchet | The published SDKs -- @hatchet-dev/typescript-sdk, hatchet-sdk on PyPI, the Go package -- are worker libraries that connect over gRPC to whichever engine a deployment configures, not clients of this REST API, so a project holding one is running tasks rather than reading the management surface. Checked 2026-09-11 |
| lemlist | The packages named for lemlist on npm and PyPI are unofficial wrappers rather than a published client, and the integrations lemlist supports are Zapier, Make and native CRM connectors, none of which appear in a dependency list as a library. Checked 2026-09-11 |
| Instantly | Instantly publishes no first-party client library, and the packages named for it on npm and PyPI are unofficial wrappers a project may or may not hold -- so a dependency list does not say this API is in use, and the word itself is common enough that a name match would be wrong far more often than right. Checked 2026-09-11 |
| Mailtrap | The mailtrap packages on npm and Packagist are sending clients for the transactional API on send.api.mailtrap.io rather than clients of the account-management surface this Recipe describes, so a name match would offer the wrong host. Checked 2026-09-11 |
| Zitadel | The published clients are generated gRPC stubs per language, and a project holding one is talking to whichever instance it was configured with -- the dependency names the protocol and never the host, which is the one thing an emulator would need. Checked 2026-09-11 |
| Umami | A project using Umami holds a script tag or the @umami/node tracker, which posts events to the collect endpoint; neither is a client of the reading API this Recipe describes, so a dependency would offer the wrong surface. Checked 2026-09-11 |
| Plausible | A project using Plausible holds a script tag or a proxy route rather than a client of this API, and the npm packages named for it are tracker wrappers that post events to /api/event rather than reading the Sites API. Checked 2026-09-11 |
| Figma | A project that talks to Figma holds either a plugin manifest, which is not a dependency, or a generic HTTP client and a personal access token -- so no package name on any registry says this API is in use. Checked 2026-09-11 |
| Todoist | Reached through its own apps and through generic HTTP clients holding a personal token; no client library on npm, Packagist or the Go module proxy calls this API under an obvious name, checked 2026-09-11. The npm results for the word are unofficial wrappers of the REST surface Todoist retired, which answers 410 |
| Sourcegraph | Reached through its CLI, its browser extension and generic GraphQL clients pointed at whichever instance an organisation runs, so a dependency name says neither that this API is used nor which host it lives on. Checked 2026-09-10 |
| Fastmail | A project speaking JMAP holds a JMAP client pointed at whichever server it discovered through autodiscovery, so the dependency names the protocol and never the host -- the same reason Matrix ships unmapped. Checked 2026-09-10 |
| Cloudsmith | Cloudsmith is driven by its CLI and by native package managers pointed at a repository URL -- apt, npm, NuGet, Docker -- rather than by a client of this management API, so a dependency list shows the package manager and never this surface. Checked 2026-09-10 |
| Matrix | The clients on npm -- matrix-js-sdk and its wrappers -- speak this protocol to whichever homeserver a deployment configures, so the dependency says the project speaks Matrix and nothing about which server it talks to. That is the one thing an emulator would need in order to stand in for it, and no dependency name carries it. Checked 2026-09-10 |
| Airship | The published libraries are named for Urban Airship and target the push *send* surface rather than the channels one this Recipe models, so a project holding one is sending notifications rather than enumerating devices. Checked 2026-09-09 |
| CloudAMQP | A project talking to CloudAMQP holds an AMQP client -- amqplib, php-amqplib, streadway/amqp -- pointed at a broker URL, not a client of this management API. The dependency says the project speaks AMQP and nothing about whether it calls api.cloudamqp.com, so a name match would offer a management emulator to code that only ever publishes messages. Checked 2026-09-09 |
| Insightly | No client for this API on npm, Packagist or the Go module proxy under an obvious name, checked 2026-09-09. The API is also pod-sharded -- api.{pod}.insightly.com -- so even a client that existed could not be pointed anywhere useful from a dependency name alone |
...
| ~~Rev.ai~~ | Shipped -- see the row above |
| ~~Agicap~~ | Shipped. **An unauthenticated request is a 500, and the reason is the rate limiter**: it partitions by client, derives its bucket key from the caller's identity, and an anonymous request has none -- so building the key throws and the framework's unhandled-error page comes back. The first call anybody makes against this API, before wiring up a credential, is a server error that every retry policy treats as transient and that will never clear. The problem `type` points at the RFC paragraph defining 500. `detail` names the internal component and describes the code path, in broken English, and is unusually true and specific. **A wrong credential is a 404**, so sending nothing is a server error, sending something wrong is a missing endpoint, and nothing this API does to a bad credential involves the status meant for one |
| ~~dLocal~~ | Shipped -- see the row above |
| msgraph | The SDKs are generated per language and named for the platform rather than the API, so no dependency name identifies this surface unambiguously, checked 2026-09-07 |
| metagraph | The Facebook SDKs are named for the platform, and a project depending on one may be using Login, Marketing or Graph |
| ~~Grafana Cloud~~ | Shipped -- see the row above |
| ~~Browserbase~~ | Shipped. **The refusal names the exact header you forgot** -- "Missing x-bb-api-key header" -- which is the single most useful thing a refusal can say and almost nothing here says it, and it matters more than usual because the header is not Authorization. The precision is only on the first failure: a wrong key answers the word Unauthorized in both `error` and `message`. **The 404 has the same three fields in the opposite order**, Fastify's handler against the application's own, which only a byte comparison notices -- the same instability Zuora has, from the same framework Infisical's 404 comes from. Success has no envelope where every failure has three fields. A running session has no end time and is already billing, and the region is per session rather than per project |
| ~~Nango~~ | Shipped -- see the row above |
| ~~Qonto~~ | Shipped. **The two credential failures differ in status, serialiser and field set**: 400 pretty-printed for a missing Authorization field and 401 compact for a wrong one, with a `trace_id` on the second only. The status pair is the right way round and almost unique here, where most providers answer one status to both -- and the failure a caller is more likely to hit anonymously is the one with nothing to quote. **An unknown path is JSON declared as `text/plain`**, the same mislabel Wrike has and in the same direction, here on the routes that do not exist rather than the ones that do. The envelope is otherwise consistent across all three, so one parser reads every failure. Money arrives twice, as a float and as an integer of cents. A pending transaction has emitted and not settled, so two balances disagree and neither is wrong |
| ~~Sift~~ | Shipped -- see the row above |
...
| Dialpad | No client on npm, Packagist or the Go module proxy under any obvious name, checked 2026-09-07 |
| Helicone | Used as a changed base URL on somebody else's SDK rather than through a client of its own, so a project that depends on it depends on the OpenAI or Anthropic package -- which no dependency name can distinguish from one that does not |
| Gong | No client on npm, Packagist or the Go module proxy under any obvious name, checked 2026-09-07 |
...
```
597 record field name(s) across 216 recipe(s) are declared and asserted
by no case.
